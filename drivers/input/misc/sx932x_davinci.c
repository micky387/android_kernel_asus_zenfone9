...
//	return 0;
	...
	if (this){
		...
		* (1->NIRQ=0, 0->NIRQ=1) */
		...
		if (this->init){
			schedule_delayed_work(&this->initworker,msecs_to_jiffies(23000));	//Delay to avoid i2c r/w fail.
		}else{
	...
	schedule_delayed_work(&this->hwcheckworker,msecs_to_jiffies(23500));	//Delay to avoid i2c r/w fail.
...
//#if IS_ENABLED(CONFIG_CAP_DEBUG)
		cap_info("Worker - Refresh Status %x",status);
//#endif		
