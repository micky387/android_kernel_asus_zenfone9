...
static const unsigned int asus_cap_extcon_cable[] = {
	EXTCON_ASUS_INTERNAL_CAP_STATUS,
	EXTCON_NONE,
};

/////////////////////////////////////////////////////////
// Debug code to trigger ext con event
#ifdef DEBUG_CAP_EXTCON
struct extcon_dev	*g_cap_satus_extcon = 0;
static ssize_t cap_extevent_write(struct file *filp, const char *buff, size_t len, loff_t *off)
{
	char messages[256];
	memset(messages, 0, sizeof(messages));

	if (!g_cap_satus_extcon){
		cap_info("ERR! g_cap_satus_extcon has not set..\n");
		return -EFAULT;
	}
	...
	if (strncmp(messages, "0", 1) == 0) {
		cap_info("trigger cap_extevent 0\n");
		extcon_set_state_sync(g_cap_satus_extcon, EXTCON_ASUS_INTERNAL_CAP_STATUS, 0);
	} else if (strncmp(messages, "1", 1) == 0) {
		cap_info("trigger cap_extevent 1\n");
		extcon_set_state_sync(g_cap_satus_extcon, EXTCON_ASUS_INTERNAL_CAP_STATUS, 1);
	} else {
		cap_info("cap_extevent.. unknown.\n");
	}

	return len;
}

static ssize_t cap_extevent_read(struct file *file, char __user *buf,
...
static struct proc_ops cap_extevent_ops = {
	.proc_write = cap_extevent_write,
	.proc_read  = cap_extevent_read,
};
#endif
//ASUS_BSP --- register cap_satus uevent
...
static int write_register(psx93XX_t this, u8 address, u8 value)
{
	...
	if (this && this->bus) {
		...
		//cap_info("write_register Address: 0x%x Value: 0x%x Return: %d",address,value,returnValue);
	}
	return returnValue;
}
...
static int read_register(psx93XX_t this, u8 address, u8 *value)
{
	...
	if (this && value && this->bus) {
		...
		#ifdef DEBUG
		cap_info("read_register Address: 0x%x Return: 0x%x",address,returnValue);
		#endif
			...
			struct device_attribute *attr,const char *buf, size_t count)
{
	...
	if (val) {
		cap_info("Performing manual_offset_calibration()");
...
static int sx932x_Hardware_Check(psx93XX_t this)
{
	...
	cap_info("%s failcode  0x%x",__func__,failcode);
	if(ret < 0){
		this->failStatusCode = SX932x_I2C_ERROR;
		cap_info("%s SX932x_I2C_ERROR",__func__);
	}
	...
	cap_info("sx932x failcode = 0x%x",this->failStatusCode);
			...
			struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%x,%x", &reg_address, &val) != 2) {
		cap_err("%s - The number of data are wrong",__func__);
		return -EINVAL;
	}

	write_register(this, (unsigned char)reg_address, (unsigned char)val);
	cap_info("%s - Register(0x%x) data(0x%x)",__func__, reg_address, val);
			...
			struct device_attribute *attr, const char *buf, size_t count)
{
	...
	cap_info("%s - Register(0x%2x) data(0x%2x)",__func__, regist, val);
...
static void read_rawData(psx93XX_t this)
{
	...
	if(this){
		for(csx =0; csx<4; csx++){
			...
			if (diff > 32767)
				diff -= 65536;
			cap_info("rawData: [CS: %d] Useful = %d Average = %d, DIFF = %d Offset = %d",csx,useful,average,diff,offset);
		}
	}
}
...
static int read_cap_status(psx93XX_t this)
{
    ...
    if (useful_ph0 >= 1800) {
        pr_info("[SX932x] Near\n");
//				extcon_set_state_sync(this->cap_satus_extcon, EXTCON_ASUS_INTERNAL_CAP_STATUS, 1);
		return 1;
    } else {
        pr_info("[SX932x] Far\n");
//				extcon_set_state_sync(this->cap_satus_extcon, EXTCON_ASUS_INTERNAL_CAP_STATUS, 0);
        return 0;
    }
}
#endif
...
#endif
	if (ret < 0) {
        cap_err("read cap register fail");
        return -2;
    }
                                                ...
                                                struct device_attribute *attr, char *buf)
{
	...
	for(reg=0x0; reg<=0x68; reg++){
	    ...
	    if (ret < 0) {
	        cap_err("read [0x%02x] fail", reg_val);
	    } else {
	        cap_info("[0x%02x]=0x%02x",reg, reg_val);
...
static void sx932x_reg_init(psx93XX_t this)
{
	...
	cap_info("Going to Setup I2C Registers");
	...
	/*******************************************************************************/
		...
		cap_err("ERROR! platform data 0x%p",pDevice->hw);
	}
	cap_info("End of Setup I2C Registers");
}
...
static int initialize(psx93XX_t this)
{
	...
	if (this) {
		cap_info("SX932x income initialize");
		...
		cap_info("Sent Software Reset. Waiting until device is back from reset to continue.");
		...
		if(err < 0)
		cap_err("%s: failed to set sx932x pinctrl active state", __func__);
...
static int sx932x_parse_dt(struct sx932x_platform_data *pdata, struct device *dev)
{
	...
	if (pdata->irq_gpio < 0) {
		cap_err("%s - get irq_gpio error", __func__);
		return -ENODEV;
	}
	...
	cap_info("%s -  size of elements %d", __func__,pdata->i2c_reg_num);
	...
	/***********************************************************************/
	cap_info("%s -[%d] parse_dt complete", __func__,pdata->irq_gpio);
	return ret;
}
...
static int sx932x_init_platform_hw(struct i2c_client *client)
{
	...
	cap_info("%s init_platform_hw start!",__func__);

	if (this && (pDevice = this->pDevice) && (pdata = pDevice->hw)) {
		if (gpio_is_valid(pdata->irq_gpio)) {
			rc = devm_gpio_request(this->pdev, pdata->irq_gpio, "sx932x_irq_gpio");
			if (rc < 0) {
				cap_err("SX932x Request gpio. Fail![%d]", rc);
				return rc;
			}
			rc = gpio_direction_input(pdata->irq_gpio);
			if (rc < 0) {
				cap_err("SX932x Set gpio direction. Fail![%d]", rc);
				return rc;
			}
			this->irq = client->irq = gpio_to_irq(pdata->irq_gpio);
			cap_info("%s : IRQ number = %d", __func__, this->irq);
		}
		else {
			cap_err("SX932x Invalid irq gpio num.(init)");
		}
	}
	else {
		cap_err("%s - Do not init platform HW", __func__);
	}
	
	cap_err("%s - sx932x_irq_debug",__func__);
	return rc;
}
...
static void sx932x_exit_platform_hw(struct i2c_client *client)
{
	...
	if (this && (pDevice = this->pDevice) && (pdata = pDevice->hw)) {
		if (gpio_is_valid(pdata->irq_gpio)) {
			...
			cap_err("Invalid irq gpio num.(exit)");
		}
	}
	return;
}
...
static void sx93XX_init_worker_func(struct work_struct *work)
{
	...
	if (work) {
		...
		if (!this) {
			cap_err("sx93XX_init_worker_func, NULL sx93XX_t");
			return;
		}
		if (this->init)
			this->init(this);
	} else {
		cap_err("sx93XX_init_worker_func, NULL work_struct");
	}
}
//ASUS_BSP LiJen --- add initial work for resume
...
static void sx93XX_hwcheck_worker_func(struct work_struct *work)
{
	...
	if (work) {
		...
		if (!this) {
			cap_err("sx93XX_hwcheck_worker_func, NULL sx93XX_t");
			return;
		}
		sx932x_Hardware_Check(this);
		
	} else {
		cap_err("sx93XX_hwcheck_worker_func, NULL work_struct");
	}
}
//ASUS_BSP LiJen --- add hwcheck work for probe
...
{    
	...
	// diable cap driver
	cap_info("Disable sx932x driver");
	return 0;
	...
	cap_info("Initialized Main Memory: 0x%p",this);
	...
	cap_err("SX932x init_platform_hw done!");

	if (this){
		cap_info("SX932x initialize start!!");
		...
		* (1->NIRQ=0, 0->NIRQ=1) */
		...
		cap_info("Initialized Device Specific Memory: 0x%p",pDevice);
	...
	//ASUS_BSP +++ register cap_satus uevent
	this->cap_satus_extcon = devm_extcon_dev_allocate(this->pdev,asus_cap_extcon_cable);
	if (IS_ERR(this->cap_satus_extcon)) {
		err = PTR_ERR(this->cap_satus_extcon);
		cap_err("failed to allocate ASUS cap_satus_extcon device rc=%d", err);
	}

	err = devm_extcon_dev_register(this->pdev, this->cap_satus_extcon);
	if (err < 0) {
		cap_err("failed to register ASUS cap_satus_extcon device rc=%d", err);
	}
	//ASUS_BSP --- register cap_satus uevent

#ifdef DEBUG_CAP_EXTCON
	g_cap_satus_extcon = this->cap_satus_extcon;
	proc_create("set-cap-extcon-event", 0666, NULL, &cap_extevent_ops);
#endif

	cap_info("sx932x_probe() Done");
...
#if IS_ENABLED(CONFIG_CAP_DEBUG)
		cap_info("Worker - Refresh Status %d",status);
#endif		
...
void sx93XX_suspend(psx93XX_t this)
{
	...
	if (this){
		...
		if(err < 0)
			cap_err("%s: failed to set sx932x pinctrl state", __func__);
	}
...
int sx93XX_IRQ_init(psx93XX_t this)
{
	...
	if (this && this->pDevice)
	{
		...
		if (err) {
			cap_err("irq %d busy?", this->irq);
			return err;
		}
		cap_info("registered with irq (%d)", this->irq);

		sx932x_pinctrl = devm_pinctrl_get(this->pdev);
		if (IS_ERR(sx932x_pinctrl)){
			cap_err("%s: failed at sx932x devm_pinctrl_get", __func__);
			return PTR_ERR(sx932x_pinctrl);
		}
		sx932x_pinctrl_state_active = pinctrl_lookup_state(sx932x_pinctrl, "sx932x_active");
		if (IS_ERR(sx932x_pinctrl_state_active)){
			cap_err("%s: failed at sx932x pinctrl_lookup_state", __func__);
			return PTR_ERR(sx932x_pinctrl_state_active);
		}
		sx932x_pinctrl_state_suspend = pinctrl_lookup_state(sx932x_pinctrl, "sx932x_suspend");
