...
#define cap_info(fmt, arg...)	pr_info("[SX932x][%s:%d] "fmt"\n", __func__, __LINE__, ##arg)
#define	cap_err(fmt, arg...)	pr_err("[SX932x][%s:%d] "fmt"\n", __func__, __LINE__, ##arg)
#define cap2_info(fmt, arg...)	pr_info("[SX932x_2nd][%s:%d] "fmt"\n", __func__, __LINE__, ##arg)
#define	cap2_err(fmt, arg...)	pr_err("[SX932x_2nd][%s:%d] "fmt"\n", __func__, __LINE__, ##arg)

//ASUS_BSP +++ register cap_satus uevent
enum CAP_STATUS_UEVENT {
	CAP_STATUS_UEVENT_FAR,
	CAP_STATUS_UEVENT_NEAR,
	CAP_STATUS_UEVENT_NONE,
};
//ASUS_BSP --- register cap_satus uevent
...
struct sx93XX 
{
	...
	* or does not work properly */
	...
	struct delayed_work initworker; /* ASUS_BSP initial work struct for worker function */
	struct delayed_work hwcheckworker; /* ASUS_BSP hwcheck work struct for worker function */
	struct extcon_dev	*cap_satus_extcon; //ASUS_BSP cap_satus uevent
};
