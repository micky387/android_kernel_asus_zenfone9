...
ssize_t sysfs_log_frames_store(struct device *dev,
	...
	if(snt8100fsr_g->chip_reset_flag == GRIP_RST_FW_DL){
		PRINT_INFO("skip log_frame set request during chip reset");
		return -1;
	}
...
ssize_t sysfs_write_vendor_fpc_file_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count) {
	if(snt8100fsr_g==NULL){
		PRINT_INFO("snt8100fsr_g==NULL");
		return count;
	}
	snt8100fsr_g->fpc_vendor_flag = grip_factory_read(VENDOR_FPC_FLAG_LOCATION);
	PRINT_INFO("data=%d", snt8100fsr_g->fpc_vendor_flag);
	return count;
}
...
ssize_t sysfs_chip_reset_store(struct device *dev,
	...
	err = request_firmware(&fw, REQUEST_GRIP_FW_PATH, dev);
	if (err) {
		PRINT_ERR("Error: request_firmware failed!!!");
		return -ENOENT;
	}
	if(fw->size > 8){
		pr_info("snt: 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x",
				fw->data[7], fw->data[6], fw->data[5], fw->data[4],
				fw->data[3], fw->data[2], fw->data[1], fw->data[0]);
	}
	return count;
}
/* Calibration write data --- */
...
static struct device_attribute grip_property_attrs[] = {
	...
	__ATTR(fpc_vendor_flag, 0220, NULL, sysfs_write_vendor_fpc_file_store),
#ifdef SUPPORT_FLASH
