...
static void grip_i2c_add(void);
//static DECLARE_DELAYED_WORK(grip_i2c_add_work, grip_i2c_add);
extern void grip_i2c_wake_add(struct work_struct *work);
extern void grip_i2c_wake_remove(struct work_struct *work);
DECLARE_DELAYED_WORK(grip_i2c_wake_add_work, grip_i2c_wake_add);
DECLARE_WORK(grip_i2c_wake_remove_work, grip_i2c_wake_remove);
void grip_i2c_add(void){
	int ret;
...
static int __init snt_fst_i2c_init(void)
{
	...
	schedule_delayed_work(&grip_i2c_wake_add_work, msecs_to_jiffies(500));
	PRINT_INFO("END");
	return 0;
}
...
static void __exit snt_fst_i2c_exit(void)
{
	...
	PRINT_INFO("EXIT");
	schedule_work(&grip_i2c_wake_remove_work);
}
