...
static int grip_2v8_regulator_init(void)
{
	...
	PRINT_INFO("vcc_gripsensor 2v8 regulator setting init");
	return ret;
}
...
static int grip_2v8_regulator_enable(void)
{
	...
	if(snt8100fsr_g->count_2v8 >= 1){
		PRINT_INFO("Sip 2v8 enable due to count=%d\n", snt8100fsr_g->count_2v8);
		return -1;
	}
	if(IS_ERR_OR_NULL(reg_2v8)){
		ret = PTR_ERR(reg_2v8);
	...
	snt8100fsr_g->count_2v8++;
	PRINT_INFO("Update vcc_gripsensor 2v8 to NPM_mode, %d", snt8100fsr_g->count_2v8);
	return ret;
}
...
static int grip_2v8_regulator_disable(void)
{
	...
	if(snt8100fsr_g->count_2v8 <= 0){
		PRINT_INFO("Sip 2v8 disable due to count=%d\n", snt8100fsr_g->count_2v8);
		return -1;
	}
	if(IS_ERR_OR_NULL(reg_2v8)){
		ret = PTR_ERR(reg_2v8);
	...
	snt8100fsr_g->count_2v8--;
	PRINT_INFO("Update vcc_gripsensor 2v8 to LPM_mode, %d", snt8100fsr_g->count_2v8);
	return ret;
}
...
static int grip_1v2_regulator_init(void)
{
	...
	PRINT_INFO("vcc_gripsensor 1v2 regulator setting init");
	return ret;
}
...
static int grip_1v2_regulator_enable(void)
{
	...
	PRINT_INFO("require 1000mA");
	ret = regulator_set_load(reg_1v2, 300000);
	if(ret < 0){
	...
	PRINT_INFO("Update vcc_gripsensor 1v2 to NPM_mode");
	return ret;
}
...
static int grip_1v2_regulator_disable(void)
{
	...
	PRINT_INFO("Update vcc_gripsensor 1v2 to LPM_mode");
	return ret;
}
...
static int grip_1v8_regulator_init(void)
{
	...
	PRINT_INFO("vcc_gripsensor 1v8 regulator setting init");
	return ret;
}
...
static int grip_1v8_regulator_enable(void)
{
	...
	PRINT_INFO("Update vcc_psensor 1v8 to NPM_mode");
	return ret;
}
...
static int grip_1v8_regulator_disable(void)
{
	...
	PRINT_INFO("Update vcc_gripsensor 1v8 to LPM_mode");
	return ret;
}
...
static int  set_1V2_2V8_pin_func_no_work(void) {
...
*/
	gpio_req = of_get_named_gpio(snt8100fsr_g->dev->of_node, SNT_ID_NAME, 0);
	gpio_request(gpio_req, "snt-id-gpio");
	gpio_direction_output(gpio_req, 1); //output high
	msleep(1);
	PRINT_INFO("ID pin pull-up, %d", snt8100fsr_g->fc_id_status);

	gpio_direction_output(gpio_req, 0); //output high
	PRINT_INFO("ID pin pull-down");
	...
	gpio_direction_output(gpio_req, 1); //output high
	PRINT_INFO("rst pin pull-up done, rst=%d, id=%d", 
	gpio_get_value(snt8100fsr_g->rst_gpio), gpio_get_value(snt8100fsr_g->snt_id_num));
	...
	if(reg_chidlsb == 0x0 && reg_chidmsb == 0x0){
		PRINT_INFO("HW_ID check fail");
		return -1;
	}else{
		PRINT_INFO("HW_ID check success");
	}
	...
	snt_set_pinctrl(snt8100fsr_g->dev, GRIP_ID_SUSPEND);
	return 1;
}
...
void asus_init_probe(void){
	...
	snt8100fsr_g->fc_id_status = 0;

	snt8100fsr_g->hw_id_status = set_1V2_2V8_pin_func_no_work();
	...
	create_Grip_Tap_vib_repeat_en_proc_file();
	create_Grip_Tap_vib_repeat_dur_proc_file();
	...
	create_Grip_Squeeze_BarA_drop_rate_proc_file();
	create_Grip_Squeeze_BarA_drop_rate_acc_proc_file();
	create_Grip_Squeeze_BarA_up_rate_proc_file();
	create_Grip_Squeeze_BarA_up_rate_acc_proc_file();
	create_Grip_Squeeze_BarB_drop_rate_proc_file();
	create_Grip_Squeeze_BarB_drop_rate_acc_proc_file();
	create_Grip_Squeeze_BarB_up_rate_proc_file();
	create_Grip_Squeeze_BarB_up_rate_acc_proc_file();
	...
	create_Grip_FW_Fail_Count_proc_file();
