...
#define ALSPS_SUSPEND 1
#define ALSPS_RESUME 0
...
{	
	...
	struct timespec64 IT_change_ts; 
	u64 evt_skip_time_ns;
	struct timespec64 prox_ts; 
...
static int proximity_turn_onoff(bool bOn)
{
	...
	if (bOn == 1)	{	/* power on */
		...
		if(true != g_ps_data->autok){
			/*irq should be enabled after autok done to prevent wrong interrupt to be triggered*/
		...
		if(g_ps_data->Device_switch_on == true){
			...
			ktime_get_ts64(&g_als_data->prox_ts);
			g_als_data->prox_off_debounce_time_ns = timespec64_to_ns(&g_als_data->prox_ts) + g_ALSPS_hw_client->mlsensor_hw->light_hw_get_evt_skip_time_ns();
		}
	}
...
static void proximity_polling_adc(struct work_struct *work)
{
	...
	if(g_ALSPS_hw_client->mpsensor_hw->proximity_hw_get_adc == NULL) {
		...
		if(g_ALSPS_status.in_suspend == ALSPS_SUSPEND){
...
static int light_turn_onoff(bool bOn)
{
	...
	if (bOn == 1)	{	/* power on */
		...
		g_ALSPS_status.als_log_first_evt = true;
	} else	{	/* power off */	
		/*set turn off register*/
		if(g_als_data->Device_switch_on == true){
			/*disable IRQ before switch off*/		
...
static int light_lux_check_psensor_noise(int lux)
{
	...
	ktime_get_ts64(&ts);
	l_current_time_ns = timespec64_to_ns(&ts);
	...
	if(g_ALSPS_status.als_log_first_evt){
...
static int light_resume_turn_on(bool bOn)
{
	int ret=0;
	if(g_ALSPS_hw_client->mlsensor_hw->light_hw_interrupt_onoff == NULL) {
		err("light_hw_interrupt_onoff NOT SUPPORT. \n");
...
static int light_get_accuracy_gain(void)
{
	...
	log("%d, %d", g_als_data->g_als_calvalue, gainvalue);
	g_als_data->g_als_accuracy_gain = gainvalue;
	
	return gainvalue;
}
static int light_log_threshold_setting(int lux){
	int light_log_threshold = 0;
	if(lux > 5000){
		light_log_threshold = LIGHT_LOG_ULTRA_HIGH_LUX_THRESHOLD;
	}else if(lux > 2000){
		light_log_threshold = LIGHT_LOG_HIGH_LUX_THRESHOLD;
	}else if(lux > 100){
		light_log_threshold = LIGHT_LOG_NORMAL_LUX_THRESHOLD;
	}else if(lux > 20){
		light_log_threshold = LIGHT_LOG_LOW_LUX_THRESHOLD;
	}else{
		light_log_threshold = LIGHT_LOG_ULTRA_LOW_LUX_THRESHOLD;
	}
...
mutex_lock(&g_alsps_lock);
	if(g_als_data->HAL_switch_on == true) {
		...
		adc = g_ALSPS_hw_client->mlsensor_hw->light_hw_get_adc();
		dbg("tmd2755 skip first event: %d, adc=%d, retry-count:%d, L-count:%d", g_ALSPS_status.als_skip_polling_first_event, adc, g_als_data->g_als_retry_count, limit_count);
		/* need polling thread to cover first event only when behavior is 0 lux */
		if (adc > 0 && g_ALSPS_status.als_skip_polling_first_event == true){
...
mutex_unlock(&g_alsps_lock);
		...
		if ((0 == adc) && (g_als_data->g_als_retry_count < limit_count)) {
			...
			/* ASUS BSP Clay: shift lux to mitigate psensor noise when psensor on and lux < offset --- */
			light_log_threshold = light_log_threshold_setting(lux);
				...
				if(g_als_last_lux != lux){
					if(lux==0){
						log("[Polling2] Light Sensor Report lux : %d (adc = %d), last_lux=%d, count=%d, poll_t=%u\n"
							, lux, adc, g_als_last_lux, log_count, polling_time);
						log_count = 0;
						g_als_last_lux = lux;
					}else if((abs(g_als_last_lux - lux) > light_log_threshold && (log_count >= (limit_count/2)))
							|| log_count >= limit_count){
						log("[Polling3] Light Sensor Report lux : %d (adc = %d), last_lux=%d, count=%d, poll_t=%u\n"
							, lux, adc, g_als_last_lux, log_count, polling_time);
						log_count = 0;
						g_als_last_lux = lux;
					}else{
...
{
	if(false == g_ALSPS_status.probe_status){
...
static void mproximity_store_load_pocket_calibration_data(int value)
{
	if(value > 0){
		g_pocket_mode_threshold = value;
		g_pocket_mode_threshold_orig = value;
	}
...
static void mproximity_store_load_near_calibration_data(int value)
{
	if(value > 0){
		g_ps_data->g_ps_calvalue_hi = value;
		g_ps_data->g_ps_factory_cal_hi = value;
	}
	log("Proximity read near Calibration : %d\n", g_ps_data->g_ps_calvalue_hi);
}

static void mproximity_store_load_far_calibration_data(int value)
{
	if(value > 0){
		g_ps_data->g_ps_calvalue_lo = value;
		g_ps_data->g_ps_factory_cal_lo = value;
	}
	log("Proximity read Low Calibration : %d\n", g_ps_data->g_ps_calvalue_lo);
}

static void mproximity_store_load_inf_calibration_data(int value)
{
	if(value >= 0){
		g_ps_data->g_ps_calvalue_inf = value;
		g_ps_data->g_ps_calvalue_inf = value;
	}
	log("Proximity read Inf Calibration : %d\n", g_ps_data->g_ps_calvalue_inf);
}
...
static int mproximity_store_pocket_en(bool flag){
	...
	if(g_ALSPS_hw_client->mpsensor_hw->proximity_hw_set_offset_limit!=NULL){
...
static psensor_ATTR_Extension mpsensor_ATTR_Extension = {
	...
	.proximity_store_load_pocket_calibration_data = mproximity_store_load_pocket_calibration_data,
	.proximity_store_load_near_calibration_data = mproximity_store_load_near_calibration_data,
	.proximity_store_load_far_calibration_data = mproximity_store_load_far_calibration_data,
	.proximity_store_load_inf_calibration_data = mproximity_store_load_inf_calibration_data,
	/*For enable anti-oil workaround*/
...
void light_work(void)
{
	...
	if(g_als_data->HAL_switch_on == true)
	{
		...
		if(light_get_lux(adc * g_als_data->dynamic_sensitive) > 100) {
		...
		if(g_ALSPS_status.als_log_first_evt){
			...
			g_ALSPS_status.als_log_first_evt = false;
			ALS_dynamic_ctl_check(lux);
			psensor_onoff_recovery(true);
		}else{
			if(ALS_check_event_rest_time(lux)==1){
				dbg("lux=%d, last_lux=%d, light_log_threshold = %d", lux, g_als_last_lux, light_log_threshold);
...
static int ALS_dynamic_ctl_check(int lux){
	if(g_ALSPS_hw_client->mlsensor_hw->light_hw_dynamic_check(lux)==1){
		...
		ktime_get_ts64(&g_als_data->IT_change_ts);
		g_als_data->evt_skip_time_ns = timespec64_to_ns(&g_als_data->IT_change_ts) + g_ALSPS_hw_client->mlsensor_hw->light_hw_get_evt_skip_time_ns();
		log("Light sensor: sense=%d, IT=%d, skip_t=%llx, cur_t=%llx, rest_t %llx", 
			...
			timespec64_to_ns(&g_als_data->IT_change_ts), g_als_data->evt_skip_time_ns);
		return 1;
	}else
		return 0;
}
...
static int proximity_check_minCT(void)
{
	...
	*/
	...
	if(crosstalk_diff>g_ps_data->g_ps_autok_min && crosstalk_diff<g_ps_data->g_ps_autok_max){
		...
		g_pocket_mode_threshold += crosstalk_diff;
		if(g_pocket_mode_threshold > PROXIMITY_PTH_MAX){
			g_pocket_mode_threshold = PROXIMITY_PTH_MAX;
		}
...
static void proximity_autok(struct work_struct *work)
{
	...
	if((crosstalk_diff<g_ps_data->crosstalk_diff) &&( g_ps_data->crosstalk_diff!=0)){
		...
		if(crosstalk_diff<=g_ps_data->g_ps_autok_min){
			...
			g_pocket_mode_threshold += (crosstalk_diff-g_ps_data->crosstalk_diff);
			g_ps_data->crosstalk_diff = crosstalk_diff;
...
static int init_data(void)
{
	...
	g_als_data->g_als_accuracy_gain = (1000*LIGHT_GAIN_ACCURACY_CALVALUE)/(g_als_data->g_als_calvalue);
...
static void mALSPS_algo_probe(struct i2c_client *client)
{
	...
	log("Driver PROBE ---\n");
	g_ALSPS_status.probe_status = true;
	return ;
probe_err:
	g_ALSPS_status.probe_status = false;
...
mutex_lock(&g_alsps_lock);
	g_ALSPS_status.in_suspend = true;
...
static void mALSPS_algo_resume(void)
{
	...
	g_ALSPS_status.in_suspend = false;

mutex_lock(&g_alsps_lock);
	if (g_als_data->Device_switch_on){
		light_resume_turn_on(true);
...
static int __init ALSPS_init(void)
{
	...
	g_ALSPS_status.probe_status = true;
	g_ALSPS_status.als_log_first_evt = false;
	g_ALSPS_status.als_skip_polling_first_event = false;
	g_ALSPS_status.in_suspend = false; //default resume
#ifdef CONFIG_TMD2755_FLAG
	g_i2c_client = NULL;
...
static void __exit ALSPS_exit(void)
{
	...
	if(g_ALSPS_status.probe_status){
