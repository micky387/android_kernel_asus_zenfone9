...
static struct work_struct openclose_check_work;
static void openclose_check_worker(struct work_struct *data);
static int openclose;
uint8_t akm_temp[8];
uint8_t akm_Ktemp[20];
unsigned int int_count=0;
extern int judgeDoorState(void);
...
void dumpI2CData(char *s, uint8_t slave_addr, uint8_t* writeBuffer,  
	...
	printk("[MCU] %s [0x%X] : len=%d, %s", s, slave_addr, numOfWriteBytes, line);
}
	...
	msgs[1].buf=rxDMA;
		...
		for (loop_i = 0; loop_i < I2C_RETRY_COUNT; loop_i++) {
			...
			if (loop_i == 0 || loop_i == I2C_RETRY_COUNT -1)
				printk("[MCU][mcp error] %s, i2c err, slaveAddr 0x%x\n",
					__func__, slave_addr);
			msleep(10);
		}
		
		if (loop_i >= I2C_RETRY_COUNT) {
			printk(KERN_ERR "[MCU][mcp error] %s slaveAddr:0x%x retry over(i2c_err) %d\n",
...
printk("[MCU] MSP430_I2CRead, slave=%d\n",slave_addr);
...
printk("[MCU] MSP430_I2CRead_NoLog, slave=%d\n",slave_addr);
...
printk("[MCU] MSP430_I2CWriteA, slave=%d\n",slave_addr);
...
printk("[MCU] MSP430_I2CWriteA_NoLog, slave=%d\n",slave_addr);
	//dumpI2CData("I2C_w", slave_addr, buffer, numberOfBytes);
	return MSP430_I2CAction(msgs, slave_addr, buffer, numberOfBytes);
}
...
uint16_t ConvertFRQMode[][19]={
	{0, 120, 120, 56, 28, 20, 12, 85, 47, 2, 2, 2, 40, 0, 0, 73, 73, 73, 73},
	{1, 120, 120, 56, 28, 20, 12, 85, 34, 2, 3, 3, 50, 0, 0, 73, 73, 73, 73},
};
...
//			printk("[MCU] detail %s", parseLine);
			for (i=1;i<count;i++) {
//				printk("[MCU] detail >>>%s", buf);
...
//					printk("[MCU] detail %s", parseLine);
		...
		{//debug
			...
			if (matched_param) {
				...
				printk("[MCU] read %s", parseLine);
			} 
		}

			
	}else {
//		printk("[MCU] No match %s", title);			
	}
...
static int MSP43FR2311_Update_Firmware_Load_File(bool bLoadFromFile) {
	...
	for (delayLoop=0;delayLoop<3;delayLoop++) {
		...
		{
			printk("[MCU] Go to bsl mode successful!\n");
			break;
		}
	}
	...
	if( res!= MSP430_STATUS_OPERATION_OK )
	{
		printk("[MCU] Go to bsl mode fail!\n");
		//msleep(5000);
		goto BSLCleanUp;
	}
...
//			udelay(2000);
			...
			{
					printk("[MCU] ERROR: Could not issue mass erase!\n");
					for (delayLoop=0;delayLoop<1000;delayLoop++)
					udelay(5000);
					goto BSLCleanUp;
			}
	...
	for (ii=0;ii<3;ii++)  {
			...
			{
				printk("[MCU] unlock device Cnt:%d\n", ii);	
				if(ii >= 3)	
					printk("[MCU] ERROR: Could not unlock device! Cnt:%d\n", ii);
...
//					tTXTFile = firmwareImage;
					while(tTXTFile != NULL)
					{
						printk("MCU txtfile assign to %p", tTXTFile);
							...
							while(ii < tTXTFile->ui32MemoryLength)
							{
									...
									if(res != MSP430_STATUS_OPERATION_OK)
									{
											printk("[MCU] FAIL!ERROR: Programming address 0x%x (Code 0x%x).\n", 
			...
			if(res != MSP430_STATUS_OPERATION_OK)
			{
					printk("[MCU] ERROR: Could not read reset vector address!\n");
					msleep(5000);
					goto BSLCleanUp;
			}
			...
			if(res != MSP430_STATUS_OPERATION_OK)
			{
					printk("[MCU] ERROR: Could not set program counter!\n");
					msleep(5000);
					goto BSLCleanUp;
			}
	
	printk("[MCU] INFO: Firmware updated without issue, %d/%d.\n", iProgrammingFail, iProgrammingCounter);
	...
	msleep(5);
	printk("[MCU] INFO: re-power on the device.\n");
	MSP430FR2311_power_control(1);
...
//			MSP430BSL_cleanUpPointer(firmwareImage);
	iProgrammingFail++;
	printk("[MCU] INFO: Programmed fail, %d/%d.\n", iProgrammingFail, iProgrammingCounter);

	//UPDATE_FW_RETRY_COUNT
	if(iProgrammingFail >= UPDATE_FW_RETRY_COUNT){
		printk("[MCU] Try 3 times to update new FW, the result is also fail.\n");
	...
	//gpio_set_value(mcu_info->mcu_5v_boost_enable, 0);
	msleep(5);
	printk("[MCU] re-power on the device when run bsl logic fail.\n");
	MSP430FR2311_power_control(1);
	//gpio_set_value(mcu_info->mcu_5v_boost_enable, 1);
	msleep(200);

	if(iProgrammingFail < UPDATE_FW_RETRY_COUNT){
		printk("[MCU] Re-try flash.\n");
...
//	for(i=0;i<2;i++, msleep(10)) 
	if ( !MSP430_I2CWriteA(MSP430_READY_I2C, getsteps, sizeof(getsteps)) |!MSP430_I2CRead(MSP430_READY_I2C, steps, sizeof(steps)) ) {
		printk("[MCU] %s I2C error!", __func__);
		return -1;
	}

	printk("[MCU] Current Steps=%d", steps[1]<<1);
	return steps[1];
}
...
void registerMCUVersion(void) {
	...
	if (IS_ERR(mcu_ver_extcon)) {
		...
		printk("[MCU] failed to allocate ASUS mcu_ver_extcon device rc=%d\n", rc);
	}
	asus_extcon_set_fnode_name(mcu_ver_extcon, "mcu");

	rc = extcon_dev_register(mcu_ver_extcon);
	if (rc < 0) {
		printk("[MCU] failed to register ASUS mcu_ver_extcon device rc=%d\n", rc);
	}
...
printk("[MCU] %s\n", __func__);
	if (MSP430FR2311_Get_Version(gFWVersion)==MSP430_STATUS_OPERATION_OK){
		...
		//}
		if (tTXTFile==NULL) {
			printk("[MCU] read firmware file error, can not to check firmware version");
			
//			printk("[MCU] seLinux security issue, workaround update firmware manually");
//			MCUState=MCU_READY;			
//			g_motor_status = 1; //probe success
			
			return MSP430_STATUS_TXTFILE_ERROR;
		}
		printk("[MCU] Firmware version=%d%02d%02d%02X, new version=%d%02d%02d%02X\n", 
			gFWVersion[0], gFWVersion[1], gFWVersion[2], gFWVersion[3],
			tTXTFile->ui8Buffer[0],
			tTXTFile->ui8Buffer[2],
		...
		if (tTXTFile->ui8Buffer[1]||
			...
			printk("[MCU] Firmware formate is incorrect\n"); 					
		}
		...
		if (
			...
			g_motor_status = 1; //probe success
			printk("[MCU] fw is newest, not need to update.\n"); 
			...
			printk("[MCU] Firmware need to be updated.\n"); 
		}
		...
		printk("[MCU] Firmware version get error.\n");
...
//	printk("[MCU] do loop test, loop=%d, wake up=%d", loop_i, loop_i&1);
//	gpio_set_value(mcu_info->mcu_wakeup, loop_i&1);
	int delay=1500;
	printk("[MCU] do loop test, loop=%d/%d", loopCounter, totalLoopCounter);
...
int MSP430FR2311_Pulldown_Drv_Power(void) {
	...
	if (MCUState<MCU_READY) {
		printk("[MCU] Not ready!, state=%d", MCUState);
		return -MCUState;
	}
...
void mcu_do_later_power_down(void) {
	...
	if (iCloseCounter!=0) {
		...
		printk("[MCU] motor power down, OpenClient=%d, CloseClient=%d", iOpenCounter, iCloseCounter);	
		iCloseCounter=0;
		iOpenCounter=0;
	} else {
		printk("[MCU] motor power down Ignore, OpenClient=%d, CloseClient=%d", iOpenCounter, iCloseCounter);	
	}
...
{		
	loopCounter++;
	printk("[MCU] %s loopCounter=%d, state=%d",__func__, loopCounter, MCUState);	
...
//			printk("[MCU] FATAL, mcu firmware update fail !!!! loopCounter=%d, state=%d", loopCounter, MCUState); 
		}
	}  else {	
		if(scinitFlg == 0){
			...
			unsigned long arg)
{
	...
	switch (cmd) {
		case ASUS_MOTOR_DRV_AUTO_MODE:
			...
			printk("[MCU] %s auto_mode:%d.\n", __func__, auto_mode);
			if(ret < 0 ){
				printk("%s: cmd = ASUS_MOTOR_DRV_AUTO_MODE, copy_from_user error(%d)\n", __func__, ret);
				goto end;
			}

			ret = MSP430FR2311_Set_AutoMode(auto_mode);
			if(ret < 0)
				printk("Set AutoMode failed\n");

			openclose = auto_mode;
			INIT_WORK(&openclose_check_work, openclose_check_worker);
			schedule_work(&openclose_check_work);
		...
		case ASUS_MOTOR_DRV_MANUAL_MODE:
			...
			if(ret < 0 ){
				printk("%s: cmd = ASUS_MOTOR_DRV_MANUAL_MODE, copy_from_user error(%d)\n", __func__, ret);
				goto end;
			}
			
			printk("[MCU] %s manual mode dir:%d, angle:%d, speed:%d.\n",  __func__, data.dir, data.angle, data.speed);
			ret = MSP430FR2311_Set_ManualMode(data.dir, data.angle, data.speed);
			if(ret < 0)
				printk("Set ManualMode failed\n");
		...
		case ASUS_MOTOR_DRV_MANUAL_MODE_NOSS:
			ret = copy_from_user(&data, (int __user*)arg, sizeof(data));
			if(ret < 0 ){
				printk("%s: cmd = ASUS_MOTOR_DRV_MANUAL_MODE_NOSS, copy_from_user error(%d)\n", __func__, ret);
				goto end;
			}
			
			printk("[MCU] %s NOSS manual mode dir:%d, angle:%d, speed:%d.\n",  __func__, data.dir, data.angle, data.speed);
			ret = ManualMode_AfterAndPR2_NOSS(data.dir, data.angle, data.speed);
			if(ret < 0)
				printk("Set NOSS ManualMode failed\n");
		...
		case ASUS_MOTOR_DRV_AUTO_MODE_WITH_ANGLE:
			...
			if(ret < 0)
				printk("Set AutoModeWithAngle failed\n");
		...
		case ASUS_MOTOR_DRV_SET_ANGLE:
			...
			if(ret < 0 ){
				printk("%s: cmd = ASUS_MOTOR_DRV_SET_ANGLE, copy_from_user error(%d)\n", __func__, ret);
				goto end;
			}
			
			printk("[MCU] %s phone set angle:%d %d.\n",  __func__, gAngle.integer, gAngle.decimals);
			wBuf[2] = 0x81;
			...
			if(ret < 0)
				printk("[MCU] Set angle to mcu failed\n");
			break;

		case ASUS_MOTOR_CALIBRATION:
			ret = copy_from_user(&cal_cmd, (int __user*)arg, sizeof(cal_cmd));			
			if(ret < 0 ){
				printk("%s: cmd = ASUS_MOTOR_CALIBRATION, copy_from_user error(%d)\n", __func__, ret);
				goto end;
			}
			...
			printk("[MCU] %s phone set calibration:%d %d.\n", __func__, cal_cmd, FacOrUserClaFlg);
			...
			if(ret < 0){
				...
				printk("[MCU] Motor calibration failed.\n");
			}else{
		...
		case ASUS_MOTOR_SPECIAL_ANGLE:
			...
			printk("[MCU] %s phone set special_angle:%d.\n", __func__, special_angle);
			if(ret < 0 ){
				printk("%s: cmd = ASUS_MOTOR_SPECIAL_ANGLE, copy_from_user error(%d)\n", __func__, ret);
				goto end;
			}
			...
			if(ret < 0)
				printk("[MCU] Motor special_angle failed.\n");
			break;

		case ASUS_MOTOR_RESET_USER_CAL:
			printk("[MCU] %s phone reset calibration value, UserKState:%d.\n", __func__, UserKState);
		...
		case ASUS_MOTOR_SAVE_USER_CAL:
			printk("[MCU] %s phone save calibration value, UserKState:%d.\n", __func__, UserKState);
		...
		case ASUS_MOTOR_NOTSAVE_USER_CAL:
			printk("[MCU] %s phone don't save calibration value, UserKState:%d.\n", __func__, UserKState);
			if(UserKState == UserK_rMCU_Ready){
		...
		case ASUS_MOTOR_ABROT_USER_CAL:
			printk("[MCU] %s phone abrot calibration, UserKState:%d.\n", __func__, UserKState);
		...
		case ASUS_MOTOR_JUDGEANGLECONDITION:
			...
			if(ret < 0 ){
				printk("%s: cmd = ASUS_MOTOR_JUDGEANGLECONDITION, copy_from_user error(%d)\n", __func__, ret);
				goto end;
			}
			...
			ret = Zen7_MSP430FR2311_wI2CtoMCU(wBuf, 4); 	//2header + 2bytes.
			if(ret < 0)
				printk("[MCU] ioctl disable judge angle failed.\n");
		
			printk("[MCU] %s phone set jasc:%d, %s jasc function.\n", __func__, JudgeStopConditionFlg, JudgeStopConditionFlg?"not use":"use");
		...
		case ASUS_MOTOR_SETUSERKTHRESHOLD:
			...
			if(ret < 0 ){
				printk("%s: cmd = ASUS_MOTOR_SETUSERKTHRESHOLD, copy_from_user error(%d)\n", __func__, ret);
				goto end;
			}
			...
			ret = Zen7_MSP430FR2311_wI2CtoMCU(wBuf, 4); 	//2header + 2bytes.
			if(ret < 0)
				printk("[MCU] UserKThreshold write failed.\n");
		
			printk("[MCU] %s phone set UserKThreshold:%d.\n", __func__, UserKThreshold);
		...
		case ASUS_MOTOR_MCU_STATE:
			...
			printk("[MCU] %s phone get mcu state:%d, ret:%d.\n", __func__, MCUState, ret);
			break;
		
		default:
			printk("[MCU][MSP430FR2311 error]%s: invalid cmd %d\n",
				__func__, _IOC_NR(cmd));
			return -EINVAL;
	}
end:
...
printk("[MCU] %s\n",__func__);
	CmdBuf[2] = 0xE0;
	if (!bPowerDone) {
		printk("[MCU] %s opening power not done, ignore this interrupt!\n", __func__);
		return;
	}
	if (!MSP430_I2CWriteA_NoLog(MSP430_READY_I2C, CmdBuf, sizeof(CmdBuf)) | !MSP430_I2CRead_NoLog(MSP430_READY_I2C, rBuf, extract_mcu_len)) {
		...
		if(IS_MASK_SET(EventState, F_Status_AKMAngleChange)){		//report angle.
			...
			if(!MSP430_I2CWriteA_NoLog(MSP430_READY_I2C, CmdBuf, sizeof(CmdBuf)) | !MSP430_I2CRead_NoLog(MSP430_READY_I2C, rBuf, 6)){
				printk("[MCU] %s cmd:0xE1 i2c read error!\n", __func__);
			}else{
...
//			report_motor_event(MOTOR_ROTATE, ROTATE_STOP);
			printk("[MCU] INT:Phone force rotate stop trigger.\n");
		}
...
//					report_motor_event(MOTOR_K_FINISH, 0);
					printk("[MCU] INT:backup calibration data trigger.\n");
				}else
					printk("[MCU] INT:%s i2c backup calibration read error!\n", __func__);			
			}

			if(IS_MASK_SET(EventState, F_Status_MotorKMCUThred)){		//Calibration and threadhold data backup.
				if(!Zen7_MSP430FR2311_rI2CtoCPU(0x70, &(fcal_val.Cal_val[0]), CalLength)){				
					backup_mcu_cal_thed();
					printk("[MCU] INT:backup theadhold data trigger.\n");
				}else
					printk("[MCU] INT:%s i2c backup theadhold read error!\n", __func__);			
			}
		}else{	//User K
			if(IS_MASK_SET(EventState, F_Status_MotorKMCUCal)){		//Calibration and threadhold data backup.
				if(UserKState == UserK_Start){
...
//						report_motor_event(MOTOR_K_FINISH, 0);
						printk("[MCU] INT:backup userK calibration data trigger.\n");
					}else
						printk("[MCU] INT:%s read userK calibration data error!\n", __func__);		
										
				}else{
					UserKState = UserK_TBD;
					printk("[MCU] INT: missing UserK interrupt.\n");
				}
		...
		if(IS_MASK_SET(EventState, F_Status_DrvInt)){		//Drv interrupt.
			...
			printk("[MCU] INT:Drv interrupt trigger.\n");
		}*/
...
//			report_motor_event(MOTOR_GET_ANGLE, 0);
			printk("[MCU] INT:Get g-sensor's angle trigger.\n");
		}
...
//			report_motor_event(MOTOR_GET_ANGLE, 1);
			printk("[MCU] INT:Get g-sensor's angle timeout trigger.\n");
		}
	}
}
...
static int init_irq(void){
	...
	if (mcu_info->mcu_irq < 0) {
		printk("[MCU] gpio_to_irq ERROR, irq=%d.\n", mcu_info->mcu_irq);
	}else {
		printk("[MCU] gpio_to_irq IRQ %d successed on GPIO:%d\n", mcu_info->mcu_irq, mcu_info->mcu_int);
	}
	...
	if (ret < 0){
		...
		printk("[MCU] request_irq() ERROR %d.\n", ret);
	}else {
		printk("[MCU] Enable irq !! \n");
...
printk("[MCU] %s+\n", __func__);
		...
		if (ret < 0) {
			printk("[MCU] %s: gpio %d request failed (%d)\n",
...
int MCU_I2C_power_control(bool enable)
{
		...
		if (IS_ERR(mcu_info->vcc_l10a_3p3))
		{
			...
			printk("[MCU] Regulator get failed vdd ret=%d", ret);
			return ret;
		}
#endif
        ...
        if (enable){
			...
			if(ret){
				printk("[MCU] Ldo11 has enabled, ingore this action.\n");
				return ret;
			}
			
			ret = regulator_enable(mcu_info->vcc_l10a_3p3);
			if(ret) 
				printk("[MCU] Ldo11 enable failed ret=%d\n", ret);       
#endif
        }else{
				...
				if(ret){
					printk("[MCU] Ldo11 is enabled, so disable it. %d \n", ret);
					
					ret = regulator_disable(mcu_info->vcc_l10a_3p3);
					if(ret){ 
						printk("[MCU] Ldo11 disable failed ret=%d\n", ret);
				
						ret = regulator_force_disable(mcu_info->vcc_l10a_3p3);
				
						if (ret)				
							printk("[MCU] Ldo11 force_disable failed, ret=%d\n", ret);
...
printk("[MCU] %s, mc_power_gpio=%d\n", __func__, gpio_get_value(mcu_info->mcu_power));
...
//			msleep(powerDownDuration); //wait motor end
		...
		#endif
		{
			...
			if (ret!=enable) {
				printk("[MCU] 3V enable failed ret=%d\n", ret);
...
int FrqConvertMode(int dir, int angle, int speed) {
	...
	if (MCUState!=MCU_READY) {
		printk("[MCU] %s Not ready!, state=%d", __func__, MCUState);
		return -MCUState;
	}
	...
	if (angle!=180){
		...
		if(angle <= 10){	 //ASUS_MOTOR_DRV_AUTO_MODE_WITH_ANGLE, angle range is less than 10 degree.
			...
			MotorDefault[10] = 0;
			MotorDefault[11] = 0;
			printk("[MCU] %s FS_steps:%d\n", __func__, FS_steps);
		}
	}else{
		//ASUS_MOTOR_DRV_AUTO_MODE or ASUS_MOTOR_DRV_AUTO_MODE_WITH_ANGLE(in condition:(mode==1 || mode==2) and angle==180).
	}
	
	printk("[MCU] PR2 auto control (Dir:%d, Angle:%d), param=%d, %d %d %d %d %d %d, %d %d %d %d %d %d, %d %d %d %d %d %d", dir, angle, MotorDefault[0],	\
...
static void openclose_check_worker(struct work_struct *data)
{
	...
	for (i=0; i<20; i++) {
		...
		delta_x = x - raw_x;
		printk("[MCU] %s, z=%d,x=%d,delta_z=%d,delta_x=%d\n",__func__,z,x,delta_z,delta_x);
		delta_z *= delta_z;
		delta_x *= delta_x;
		if ((delta_z+delta_x) > 160000) {
			msleep(200);
		} else {
			if(1==openclose) {
			...
			report_motor_event(MOTOR_ROTATE, ROTATE_FINISH);
			printk("[MCU] %s, rotate finish\n",__func__);
			break;
		}
	}
	if (20 == i) {
		report_motor_event(MOTOR_ROTATE, ROTATE_FAIL);
		printk("[MCU] %s, rotate fail\n",__func__);
	}
}
...
int MSP430FR2311_Set_AutoModeWithAngle(int mode, int angle) {
	...
	printk("[MCU] Not supported mode for %d", mode);
...
//			printk("[MCU] Power duration += (%d-%d)*2400*500/%d/300 =   %d", vals[i+6], ((i==1)?0:vals[i+5]), speed, powerDownDuration);
		};
		printk("[MCU] Power duration=%d(ms), reference only", powerDownDuration);
		powerDownDuration=DEFAULT_POWERDOWNDURATION;
		bShowStopInfoOnce=1;
...
//		printk("[MCU] dump param=%d %d %d %d %d %d %d %d %d %d %d %d %d 254", 
...
int ManualMode_AfterAndPR2(int dir, int angle, int speed) {
	...
	if(angle!=180){
		...
		MotorDefault[10]= 0;
		MotorDefault[11]= 0;
		printk("[MCU] PR2 manual control (dir:%d, angle:%d, speed:%d, manual_steps:%d, gear_step:%d).", dir, angle, speed, manual_steps, gearStep);
	}else{
		printk("[MCU] PR2 manual control (dir:%d, angle:%d, speed:%d).", dir, angle, speed);
	}
	old_dir=dir;	
	...
	if(speed <= 10){//micro-step process
		...
		printk("[MCU] %s don't support this(%d) manual speed.", __func__, speed);
	}
...
int ManualMode_AfterAndPR2_NOSS(int dir, int angle, int speed) {
	...
	MotorDefault[10]= 0;
	MotorDefault[11]= 0;
	printk("[MCU] NOSS PR2 manual control (dir:%d, angle:%d, speed:%d, manual_steps:%d, gear_step:%d).", dir, angle, speed, manual_steps, gearStep);
...
static int MSP430FR2311_power_init(void)
{
		...
		printk("[MCU] power init, set requlator.");
	    ...
	    printk("[MCU] Regulator get success.");
        ...
        if (regulator_count_voltages(mcu_info->vcc_l10a_3p3) > 0)
        {
                ...
                if (ret)
                {
                        printk("[MCU] Regulator set_vtg failed ret=%d", ret);
                        goto reg_vdd_put;
                }else
			printk("[MCU] Regulator set voltage success.");
        }else
	    printk("[MCU] Regulator count voltage fail.");
#endif
	...
	if (regulator_count_voltages(mcu_info->vcc_s4a_1p8) > 0)
	{
					...
					if (ret)
					{
									printk("[MCU] Regulator set_vtg failed vcc_s4a_1p8 ret=%d", ret);
									goto reg_vdd_put;
					}
	}
	*/
	...
	const struct i2c_device_id *id)
{
	...
	printk("[MCU] driver probe.\n");

#ifdef ALLEN
	if (g_Charger_mode == 1) {
		printk("[MCU] in charging mode, skip MCU probe\n");
	...
	if( MSP430FR2311_parse_dt(&client->dev) < 0 )
	{
		...
		//gpio_set_value(mcu_info->mcu_power, 1);
        if (ret < 0) {
                printk("[MCU] %s: set regulator fail\n", __func__);
        	}
        ...
        if (ret < 0) {
                printk("[MCU] %s: enable regulator fail\n", __func__);
								goto err_platform_data_null;	
        	}

//	if (MSP430FR2311_Check_Version() && MSP43FR2311_Go_BSL_Mode()) {
		MCUState=MCU_EMPTY;
//		printk("[MCU][MSP430FR2311 ]%s: Fail\n", __func__);		
...
//	if (MCUState!=MCU_READY) {
		...
		if (!mcu_info->mcu_wq) {
			printk("[MCU] %s: can't create workqueue\n", __func__);
			ret = -ENOMEM;
		...
		if (!mcu_info->cal_wq) {
			printk("[MCU] %s: can't create cal_wq workqueue\n", __func__);
			ret = -ENOMEM;
			goto err_create_singlethread_workqueue;
		}
...
//	if (ret < 0) {
//		printk("[MCU][MSP430FR2311 error]%s: MSP430FR2311_setup error!\n", __func__);
...
static int __init MSP430FR2311_init(void)
{
	...
	if (ret)
		printk("[MCU] MSP430 driver init failed.\n");
	else
		printk("[MCU] MSP430 driver init success.\n");
...
static int file_op(const char *filename, loff_t offset, char *buf, int length, int operation)
{
	...
	if (FILE_OP_READ == operation)
		...
		////ksys_close(filep);
		set_fs(old_fs);
		printk("[MCU] Unknown partition op err!\n");
		return -1;
	}
	if (IS_ERR(filep)) {
		//ksys_close(filep);
		set_fs(old_fs);
		printk("[MCU] open %s err! error code:%d\n", filename, filep);
		return -1;
	}
	else {
		printk("[MCU] open %s success!\n", filename);
	}
...
static int read_mcu_cal_thed(void)
{
	...
	printk("[MCU] %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x...%x %x %x %x, %x %x %x %x %x %x.\n", \
		fcal_val.Cal_val[0], fcal_val.Cal_val[1], fcal_val.Cal_val[2], fcal_val.Cal_val[3], fcal_val.Cal_val[4],	\
		fcal_val.Cal_val[5], fcal_val.Cal_val[6], fcal_val.Cal_val[7], fcal_val.Cal_val[8], fcal_val.Cal_val[9],	\
		fcal_val.Cal_val[10], fcal_val.Cal_val[11], fcal_val.Cal_val[12], fcal_val.Cal_val[13], fcal_val.Cal_val[14],	\
		fcal_val.Cal_val[15], fcal_val.Cal_val[16], fcal_val.Cal_val[17], fcal_val.Cal_val[18], fcal_val.Cal_val[19],	\
...
printk("[MCU] %s\n",__func__);
	for(i=0; i<23; i++){	//20 bufferMag + 3 rawAK9970Corr.
		...
		if(MSP430_I2CRead(MSP430_READY_I2C, rRawBuf, RawLength)){
			...
			printk("[MCU] %s i2c raw backup (%d) read error!\n", __func__, i);
	}
...
static int CompareCaliData(char *filename)
{
	...
	for(i=0; i<CalLength; i++){
		...
		if(fcal_val_rPhone.Cal_val[i] != fcal_val_rMCU.Cal_val[i]){
			wFlg = 1;
			printk("[MCU] %s i:%d, rPhone:%x, rMCU:%x.\n", __func__, i, fcal_val_rPhone.Cal_val[i], fcal_val_rMCU.Cal_val[i]);
		}
	}
	
	if(wFlg == 0){
		printk("[MCU] %s no need update phone's cali data to mcu.\n", __func__);
	}else
		Zen7_MSP430FR2311_wI2CtoMCU(wBuf, sizeof(wBuf));
...
int Zen7_MSP430FR2311_Set_ParamMode(const uint16_t* vals) {
	...
	printk("[MCU] dump param=%d, %d %d %d %d %d %d, %d %d %d %d %d %d, %d %d %d %d %d %d\n", 
			vals[0], vals[1], vals[2], vals[3], vals[4], vals[5], vals[6], vals[7], vals[8], vals[9], vals[10], vals[11], vals[12],
			vals[13], vals[14], vals[15], vals[16], vals[17], vals[18]);

	printk("[MCU] %d, %d %d %d %d %d %d, %d %d %d %d %d %d, %d %d %d %d %d %d\n", 
	...
	MSP430FR2311_wakeup(1);
	if (MCUState<MCU_READY) {
		printk("[MCU] %s Not ready!, state=%d\n", __func__, MCUState);
		mutex_unlock(&MSP430FR2311_control_mutex);
		return -MCUState;
	}
...
printk("[MCU] %s\n", __func__);
	...
	if(kobject_uevent_env(&((mcu_misc.this_device)->kobj), KOBJ_CHANGE, envp) != 0){
		printk("[MCU] kobject_uevent_env fail...\n");
	} else {
...
printk("[MCU] %s\n", __func__);
	...
	if(ret < 0)
		printk("[MCU] mStopConditionInit failed.\n");
	else
		printk("[MCU] mStopConditionInit ok.\n");
...
void WQ_Trigger(void){
	...
	printk("[MCU] %s.\n", __func__);
	if (1 == bProbeFail) {
		printk("[MCU] probe fail. Don't queue work to avoid data abort!\n");
		return;
	}
	if(mcu_info->mcu_wq){
		cancel_delayed_work(&report_work);
		queue_delayed_work(mcu_info->mcu_wq, &report_work, msecs_to_jiffies(10));
		printk("[MCU] %s success.\n", __func__);
	}
}
