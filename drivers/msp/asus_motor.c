...
#define PROC_MOTOR_AKM_RAW_Z    "driver/motor_akm_raw_z"
#define PROC_MOTOR_AKM_RAW_Y    "driver/motor_akm_raw_y"
#define PROC_MOTOR_AKM_RAW_X    "driver/motor_akm_raw_x"
#define PROC_MOTOR_INT          "driver/motor_int"
#define PROC_MOTOR_DOOR         "driver/motor_door_state"
#define PROC_ROG6_K             "driver/rog6_k"
...
static int motor_akm_raw_z_read(struct seq_file *buf, void *v)
{
	...
	if (rc == 0) {
		...
		seq_printf(buf, "[MCU] Get akm raw data z fail!\n");
	}
	return 0;
}
...
static int motor_akm_raw_y_read(struct seq_file *buf, void *v)
{
	...
	if (rc == 0) {
		...
		seq_printf(buf, "[MCU] Get akm raw data y fail!\n");
	}
	return 0;
}
...
static int motor_akm_raw_x_read(struct seq_file *buf, void *v)
{
	...
	if (rc == 0) {
		...
		seq_printf(buf, "[MCU] Get akm raw data x fail!\n");
	}
	return 0;
}
...
{
	seq_printf(buf, "[MCU] rog6 k raw data %d %d %d %d\n", open_z, open_x, close_z, close_x);
	return 0;
}
...
static const struct proc_ops motor_door_fops = {
...
static void create_motor_proc_files_factory(void)
{
	...
	if(!has_created)
	{
		...
		create_proc_file(PROC_MOTOR_AKM_RAW_Z, &motor_akm_raw_z_fops);
		create_proc_file(PROC_MOTOR_AKM_RAW_Y, &motor_akm_raw_y_fops);
		create_proc_file(PROC_MOTOR_AKM_RAW_X, &motor_akm_raw_x_fops);
		create_proc_file(PROC_MOTOR_INT, &motor_int_fops);
		create_proc_file(PROC_MOTOR_DOOR, &motor_door_fops);
		create_proc_file(PROC_ROG6_K, &rog6_k_fops);
