...
#ifdef ASUS_HAPTIC_PORTING
static uint8_t aw8692x_get_gain(struct aw_haptic *aw_haptic)
{
	int ret = -1;
	uint8_t reg_val = 0;
	
	ret = i2c_r_bytes(aw_haptic, AW8692X_REG_PLAYCFG2, &reg_val, AW_I2C_BYTE_ONE);

	if (ret < 0)
		aw_err("read gain fail");
...
static void aw8692x_set_bst_vol(struct aw_haptic *aw_haptic, uint32_t bst_vol)
{
	...
	i2c_w_bits(aw_haptic, AW8692X_REG_PLAYCFG1,
		   AW8692X_BIT_PLAYCFG1_BST_VOUT_VREFSET_MASK, reg_val);
}
...
static int aw8692x_wait_enter_standby(struct aw_haptic *aw_haptic)
{
	...
	while (count--) {
		...
		if (reg_val == AW8692X_BIT_GLBRD5_STATE_STANDBY) {
			aw_info("entered standby! (%d/%d)", (100 - count), 100);
			return 0;
		}
...
#ifdef ASUS_HAPTIC_PORTING
static int asus_aw8692x_rtp_count = 0;
#endif
static int aw8692x_get_irq_state(struct aw_haptic *aw_haptic)
{
	...
	if (reg_val & AW8692X_BIT_SYSINT_FF_AFI) {
		...
		asus_aw8692x_rtp_count++;
		if (asus_aw8692x_rtp_count < 10000){
			if ( asus_aw8692x_rtp_count == 1 )
				aw_info("aw_haptic rtp mode fifo almost full!(%d)",asus_aw8692x_rtp_count);
			else if ( (asus_aw8692x_rtp_count % 50) == 0 )
				aw_info("aw_haptic rtp mode fifo almost full!(%d)",asus_aw8692x_rtp_count);
		} else if (asus_aw8692x_rtp_count == 10000) {
			aw_info("aw_haptic rtp mode fifo almost full!(%d). reset count",asus_aw8692x_rtp_count);
			asus_aw8692x_rtp_count = 0;
		}
...
static int aw8692x_get_f0(struct aw_haptic *aw_haptic)
{
	...
	i2c_w_bytes(aw_haptic, AW8692X_REG_CONTCFG7, cont_config,
		    AW_I2C_BYTE_THREE);
	/* TRACK_MARGIN */
	...
	i2c_w_bits(aw_haptic, AW8692X_REG_DETCFG2,
		   AW8692X_BIT_DETCFG2_D2S_GAIN_MASK, d2s_gain_default);
...
static void aw8692x_misc_para_init(struct aw_haptic *aw_haptic)
{
	...
	/* Get gain */
	i2c_r_bytes(aw_haptic, AW8692X_REG_PLAYCFG2, reg_val, AW_I2C_BYTE_ONE);
	aw_haptic->gain = reg_val[0];
	#ifdef ASUS_HAPTIC_PORTING
	aw_haptic->gain_trig = aw_haptic->gain;
	#endif
	/* Get wave_seq */
	i2c_r_bytes(aw_haptic, AW8692X_REG_WAVCFG1, reg_val, AW_I2C_BYTE_EIGHT);
	...
	reg_val[0] = AW8692X_BIT_PWMCFG1_INIT_VAL;
