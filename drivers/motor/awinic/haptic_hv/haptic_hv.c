...
#define HAPTIC_HV_DRIVER_VERSION	"v0.7.0"
...
#ifdef ASUS_HAPTIC_PORTING
	cpu_latency_qos_add_request(&aw_haptic->aw_pm_qos_req_vb, AW_PM_QOS_VALUE_VB);
#else
...
static void vibrator_work_routine(struct work_struct *work)
{
	...
	if (aw_haptic->state) {
		...
		schedule_delayed_work(&aw_haptic->asus_gain_work, msecs_to_jiffies(100));
	}
	#endif
	mutex_unlock(&aw_haptic->lock);
}
...
static int rtp_play(struct aw_haptic *aw_haptic)
{
	...
	while ((!aw_haptic->func->rtp_get_fifo_afs(aw_haptic))
		...
		if (aw_haptic->rtp_cnt == aw_rtp->len){
			...
			if (rtp_repeat){
				aw_haptic->rtp_cnt = 1;
				aw_info("rtp_repeat=%d",rtp_repeat);
			}
		}
		#endif
		...
		if ((aw_haptic->rtp_cnt == aw_rtp->len)
			...
			#ifdef ASUS_HAPTIC_PORTING
			schedule_delayed_work(&aw_haptic->asus_gain_work, msecs_to_jiffies(150));
			#endif
...
//=================
#ifdef ASUS_HAPTIC_PORTING
static void asus_haptic_gain_routine(struct work_struct *work)
{
	struct aw_haptic *aw_haptic = g_aw_haptic;

	//aw_info("gain=%d gain_trig=%d", aw_haptic->gain, aw_haptic->gain_trig);
	aw_haptic->gain=aw_haptic->gain_trig;
	aw_haptic->func->set_gain(aw_haptic, aw_haptic->gain_trig);
}
/*
int asus_haptic_load_rtp_file(struct aw_haptic *aw_haptic)
{
	int ret = -1;
	const struct firmware *rtp_file;
	...
	// fw loaded
//ASUS bin
	sprintf(fwName, "aw_%d.bin", aw_haptic->rtp_file_num);
			 ...
			 char *buf)
{
	...
	if (aw_haptic->chipid == AW86927_CHIPID) {
		gain = aw_haptic->func->get_gain(aw_haptic);
		aw_info("gain=%d reg=%d", aw_haptic->gain, gain);
	} else {
		aw_info("gain=%d", aw_haptic->gain);
	}
			  ...
			  const char *buf, size_t count)
{
	...
	mutex_lock(&aw_haptic->lock);
	#ifdef ASUS_HAPTIC_PORTING
	cancel_delayed_work(&aw_haptic->asus_gain_work);
	#endif
	aw_haptic->gain = val;
	aw_haptic->func->set_gain(aw_haptic, aw_haptic->gain);
	mutex_unlock(&aw_haptic->lock);
...
mutex_lock(&aw_haptic->rtp_lock);
	cancel_delayed_work(&aw_haptic->asus_gain_work);
	cancel_delayed_work(&aw_haptic->rtp_work_d);
	...
	aw_haptic->rtp_init = false;
	vfree(aw_haptic->aw_rtp);
	aw_haptic->aw_rtp = vmalloc(val + sizeof(int));
	...
	aw_rtp_offset = 0;
	aw_haptic->aw_rtp->len = val;
	aw_info("set rtp len = %d bytes", aw_haptic->aw_rtp->len);
mutex_unlock(&aw_haptic->rtp_lock);
...
//aw_info("1 rtp file size = %d bytes", rtp_file->size);
aw_info("data 1  [%x] [%x] [%x] [%x]", 
...
release_firmware(rtp_file);
*/
//	aw_info("2 rtp file size = %d bytes", aw_rtp->len);
	memcpy(aw_haptic->aw_rtp->data + aw_rtp_offset, buf, count );
/*	
	aw_info("data 2  %d %d = [%x] [%x] [%x] [%x]",
	count,
	aw_rtp_offset,
...
*/
	...
	if (aw_rtp_offset == aw_haptic->aw_rtp->len)
		aw_info("recevice rtp data complete");
...
static ssize_t asus_rtp_play_show(struct device *dev,
			       struct device_attribute *attr, char *buf)
{
	ssize_t len = 0;

	len += snprintf(buf + len, PAGE_SIZE - len, "fwName: %s\n",
			 fwName);
	return len;
}

static ssize_t asus_rtp_play_repeat_show(struct device *dev,
			       struct device_attribute *attr, char *buf)
{
	ssize_t len = 0;
	
	len += snprintf(buf + len, PAGE_SIZE - len, "fwName:%s  repeat: %d\n",
			 fwName, rtp_repeat);
	return len;
}

static ssize_t asus_rtp_play_break_show(struct device *dev,
			       struct device_attribute *attr, char *buf)
{
	cdev_t *cdev = dev_get_drvdata(dev);
	struct aw_haptic *aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);
	...
	len += snprintf(buf + len, PAGE_SIZE - len, "value: %d\n",
			aw_haptic->rtp_file_num);

	return len;
}

static int asus_rtp_play_store_base(struct device *dev,
			    unsigned int num, unsigned int repeat, unsigned int delay_ms)
{
	cdev_t *cdev = dev_get_drvdata(dev);
	struct aw_haptic *aw_haptic = container_of(cdev, struct aw_haptic,
						   vib_dev);
	...
	aw_info("rtp: [%d], repeat=%d, delay=%d", num, repeat, delay_ms);
	if((num <= 0) || (repeat <= 0)){
		aw_err("invalid file number or repeat !");
		return rc;
	}
...
//	aw_haptic->func->irq_clear(aw_haptic);
	...
	rtp_repeat = repeat;
	aw_haptic->rtp_file_num = num;
	aw_haptic->state = 1;
	//rc = asus_haptic_load_rtp_file(aw_haptic);

	//if (rc == 0)
	schedule_delayed_work(&aw_haptic->rtp_work_d, msecs_to_jiffies(delay_ms));
	
	mutex_unlock(&aw_haptic->lock);
	return rc;
}

static ssize_t asus_rtp_play_store(struct device *dev,
				...
				size_t count)
{
	...
	rc = kstrtouint(buf, 0, &val);
	if (rc < 0) {
		aw_info("kstrtouint fail");
		return rc;
	}

	asus_rtp_play_store_base( dev, val, 1, 0);
	return count;
}

static ssize_t asus_rtp_play_repeat_store(struct device *dev,
				struct device_attribute *attr, const char *buf,
				size_t count)
{
	unsigned int val = 0;
	...
	if ( rc != 2 ) {
		aw_info("invalid input! rc=%d", rc);
		return count;
	}

	asus_rtp_play_store_base( dev, val, repeat, 0);
	return count;
}

static ssize_t asus_rtp_play_delay_store(struct device *dev,
				struct device_attribute *attr, const char *buf,
				size_t count)
{
	unsigned int val = 0;
	...
	asus_rtp_play_store_base( dev, val, 1, delay);
	return count;
}

static ssize_t asus_rtp_play_break_store(struct device *dev,
				struct device_attribute *attr, const char *buf,
				size_t count)
{
	cdev_t *cdev = dev_get_drvdata(dev);
	...
	aw_info("%s: val == %d", __func__, val);
	if(val) {
		aw_err("val sould set to 0 in current function!");
		goto rtp_play_break_store_brk1;
	}

	mutex_lock(&aw_haptic->lock);

	cancel_delayed_work_sync(&aw_haptic->rtp_work_d);
	...
	rtp_repeat = 0;
	schedule_delayed_work(&aw_haptic->asus_gain_work, msecs_to_jiffies(1));
	aw_info("rtp play is broken");
...
static ssize_t asus_mem_play_show(struct device *dev,
			       struct device_attribute *attr, char *buf)
{
	cdev_t *cdev = dev_get_drvdata(dev);
	struct aw_haptic *aw_haptic = container_of(cdev, struct aw_haptic,
	...
	count += snprintf(buf, PAGE_SIZE,	"seq[0]: 0x%02x\n", aw_haptic->seq[0]);
	aw_info("waveform id == %d", aw_haptic->seq[0]);

	return count;
}

static ssize_t asus_mem_play_store(struct device *dev,
				struct device_attribute *attr, const char *buf,
				size_t count)
{
	cdev_t *cdev = dev_get_drvdata(dev);
	...
	aw_info("waveform id == %d", val);

	if(val == 0) {
		aw_err("waveform id sould not set to 0 in current function!");
	...
	cancel_delayed_work(&aw_haptic->asus_gain_work);
			...
			aw_haptic->func->play_stop(aw_haptic);
			aw_haptic->func->play_mode(aw_haptic, AW_RAM_MODE);
			aw_haptic->func->set_wav_seq(aw_haptic, 0x00, (unsigned char)val);
			aw_haptic->func->set_wav_seq(aw_haptic, 0x01, 0x00);
			aw_haptic->func->set_wav_loop(aw_haptic, 0x00, 0x00);
			//aw_haptic->func->set_gain(aw_haptic,aw_haptic->haptic_audio.ctr.gain);
			aw_haptic->func->play_go(aw_haptic, true);

	//ASUS_BSP: mem_play complete. set gain back for trig mode
	schedule_delayed_work(&aw_haptic->asus_gain_work, msecs_to_jiffies(100));
	mutex_unlock(&aw_haptic->lock);
...
static ssize_t asus_gain_trig_show(struct device *dev, struct device_attribute *attr,
			 char *buf)
{
	cdev_t *cdev = dev_get_drvdata(dev);
	struct aw_haptic *aw_haptic = container_of(cdev, struct aw_haptic,
	...
	if (aw_haptic->chipid == AW86927_CHIPID) {
		gain = aw_haptic->func->get_gain(aw_haptic);
		aw_info("gain_trig=%d reg=%d", aw_haptic->gain_trig, gain);
	} else {
		aw_info("gain_trig=%d", aw_haptic->gain_trig);
	}
	return snprintf(buf, PAGE_SIZE, "0x%02X\n", aw_haptic->gain_trig);	
}

static ssize_t asus_gain_trig_store(struct device *dev, struct device_attribute *attr,
			  const char *buf, size_t count)
{
	cdev_t *cdev = dev_get_drvdata(dev);
	struct aw_haptic *aw_haptic = container_of(cdev, struct aw_haptic,
...
static ssize_t asus_haptic_trig_control_store(struct device *dev,
				 struct device_attribute *attr, const char *buf, size_t count)
{
	cdev_t *cdev = dev_get_drvdata(dev);
	struct aw_haptic *aw_haptic = container_of(cdev, struct aw_haptic,	vib_dev);
	...
	sscanf(buf, "%d %d", &num, &enable);

	if(aw_haptic == NULL){
		aw_info("aw_haptic is null");
		ret=-1;
	} else if (aw_haptic->ram_init == 0){
		aw_info("aw haptic ram init is not complete");
		ret=-1;
	} else
		aw_info("num=%d enable=%d", num, enable);
	...
	switch(num){
	case 1:
		aw_haptic->info.trig_cfg[2] = enable;
		break;
	case 2:
		aw_haptic->info.trig_cfg[10] = enable;
		break;
	default:
		aw_info("num=%d is not support");
		ret=-1;
		break;
	}
	...
	if(ret == 0) {
		aw_haptic->func->trig_init(aw_haptic);
		aw_info("trig mode setting update!");
	}
	return count;
}

static ssize_t asus_haptic_trig_control_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	cdev_t *cdev = dev_get_drvdata(dev);
	struct aw_haptic *aw_haptic = container_of(cdev, struct aw_haptic,	vib_dev);
	...
	if(aw_haptic == NULL){
		...
		len += snprintf(buf + len, PAGE_SIZE - len,"%d %d %d\n", ret, aw_haptic->info.trig_cfg[2],aw_haptic->info.trig_cfg[10]);
			 ...
			 char *buf)
{
	...
	mutex_lock(&aw_haptic->lock);
	rc=f0_cali(aw_haptic);
	mutex_unlock(&aw_haptic->lock);
if (rc)
	len += snprintf(buf + len, PAGE_SIZE - len, "get f0 fail\n");
else
...
#ifdef ASUS_HAPTIC_PORTING
	aw_info("set f0_cali_data=%d",aw_haptic->f0_cali_data);
#endif
	return count;
}
...
#ifdef ASUS_HAPTIC_PORTING
	aw_info("set osc_cali_data=%d",aw_haptic->osc_cali_data);
#endif
	return count;
}
...
static DEVICE_ATTR(rtp_play, S_IWUSR | S_IRUGO, asus_rtp_play_show, asus_rtp_play_store);
static DEVICE_ATTR(rtp_play_repeat, S_IWUSR | S_IRUGO, asus_rtp_play_repeat_show, asus_rtp_play_repeat_store);
static DEVICE_ATTR(rtp_play_delay, S_IWUSR | S_IRUGO, NULL, asus_rtp_play_delay_store);
static DEVICE_ATTR(rtp_play_break, S_IWUSR | S_IRUGO, asus_rtp_play_break_show, asus_rtp_play_break_store);
static DEVICE_ATTR(mem_play, S_IWUSR | S_IRUGO, asus_mem_play_show, asus_mem_play_store);
static DEVICE_ATTR(gain_trig, S_IWUSR | S_IRUGO, asus_gain_trig_show, asus_gain_trig_store);
static DEVICE_ATTR(asus_haptic_trig_control, S_IWUSR | S_IRUGO, asus_haptic_trig_control_show, asus_haptic_trig_control_store);
#endif
...
static struct attribute *vibrator_attributes[] = {
	...
	&dev_attr_asus_haptic_trig_control.attr,
#endif	
...
#endif
#ifdef ASUS_HAPTIC_PORTING
	INIT_DELAYED_WORK(&aw_haptic->rtp_work_d, rtp_work_routine);
	INIT_DELAYED_WORK(&aw_haptic->asus_gain_work, asus_haptic_gain_routine);
#endif
...
static void haptic_init(struct aw_haptic *aw_haptic)
{
	...
	/* vbat compensation */
	aw_haptic->func->vbat_mode_config(aw_haptic, AW_CONT_VBAT_SW_COMP_MODE);
#ifdef ASUS_HAPTIC_PORTING
	aw_haptic->ram_vbat_comp = AW_RAM_VBAT_COMP_DISABLE;
#else
	aw_haptic->ram_vbat_comp = AW_RAM_VBAT_COMP_ENABLE;
...
{
	int ret = 0;
//printk("[haptic] aw_haptic driver version %s\n", HAPTIC_HV_DRIVER_VERSION);
	pr_info("<%s>%s: aw_haptic driver version %s\n", AW_I2C_NAME, __func__,
		HAPTIC_HV_DRIVER_VERSION);
	aw_dbg("aw_dbg working");
	aw_err("aw_err working");
	aw_info("aw_info working");
