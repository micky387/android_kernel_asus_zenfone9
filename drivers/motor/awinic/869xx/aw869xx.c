 ...
 *
 ******************************************************/
#define AW869XX_DRIVER_VERSION "v1.0.1"
	...
	while (cnt < AW_I2C_RETRIES) {
		...
		if (ret < 0) {
			aw_dev_err(aw869xx->dev,
				   "%s: addr=0x%02X, data=0x%02X, cnt=%d, error=%d\n",
...
static int aw869xx_haptic_get_ram_number(struct aw869xx *aw869xx)
{
	unsigned char i = 0;
	unsigned char reg_val = 0;
	...
	aw869xx->ram.ram_num =
			(first_wave_addr - aw869xx->ram.base_addr - 1) / 4;
	aw_dev_info(aw869xx->dev,
...
static int aw869xx_container_update(struct aw869xx *aw869xx,
...
*/
	...
	for (i = shift; i < aw869xx_cont->len; i++) {
		...
		if (reg_val != aw869xx_cont->data[i]) {
			...
			ram_check_err_flag = false;
		}
		}
		check_sum += reg_val;
	}
	aw869xx_i2c_read(aw869xx, AW869XX_REG_RTPCFG1, &reg_val);
	check_sum += reg_val;
	aw869xx_i2c_read(aw869xx, AW869XX_REG_RTPCFG2, &reg_val);
	check_sum += reg_val;

	if (check_sum != aw869xx->ram.check_sum) {
		aw_dev_err(aw869xx->dev,
			   "%s: ram data check sum error, check_sum=0x%04x\n",
			   __func__, check_sum);
		ret = -1;
	} else {
		aw_dev_info(aw869xx->dev,
			   "%s: ram data check sum pass, check_sum=0x%04x\n",
			   __func__, check_sum);
	}
#endif
	/* RAMINIT Disable */
 ...
 */
	...
	release_firmware(cont);
	ret = aw869xx_container_update(aw869xx, aw869xx_fw);
	if (ret) {
		aw_dev_err(aw869xx->dev, "%s: ram firmware update failed!\n",
			   __func__);
	} else {
		aw869xx->ram_init = 1;
	...
	aw869xx_haptic_get_ram_number(aw869xx);
	aw869xx->ram.len = aw869xx_fw->len - aw869xx->ram.ram_shift;
	kfree(aw869xx_fw);
}
...
static int aw869xx_ram_update(struct aw869xx *aw869xx)
{
	...
	add_bin_search_path(INTERNAL_BIN_PATH);
 ...
 *
 *****************************************************/
static void aw869xx_haptic_trig1_param_init(struct aw869xx *aw869xx)
{
	aw_dev_info(aw869xx->dev, "%s enter\n", __func__);
...
static void aw869xx_haptic_trig2_param_init(struct aw869xx *aw869xx)
{
	aw_dev_info(aw869xx->dev, "%s enter\n", __func__);
	...
	aw869xx->trig[1].pos_sequence = aw869xx->info.trig_config[8 + 3];
	aw869xx->trig[1].neg_enable = aw869xx->info.trig_config[8 + 4];
	aw869xx->trig[1].neg_sequence = aw869xx->info.trig_config[8 + 5];
	aw869xx->trig[1].trig_brk = aw869xx->info.trig_config[8 + 6];
	aw869xx->trig[1].trig_bst = aw869xx->info.trig_config[8 + 7];
}

static void aw869xx_haptic_trig3_param_init(struct aw869xx *aw869xx)
{
	aw_dev_info(aw869xx->dev, "%s enter\n", __func__);
	...
	aw869xx->trig[2].pos_sequence = aw869xx->info.trig_config[16 + 3];
	aw869xx->trig[2].neg_enable = aw869xx->info.trig_config[16 + 4];
	aw869xx->trig[2].neg_sequence = aw869xx->info.trig_config[16 + 5];
	aw869xx->trig[2].trig_brk = aw869xx->info.trig_config[16 + 6];
	aw869xx->trig[2].trig_bst = aw869xx->info.trig_config[16 + 7];
}

static void aw869xx_haptic_trig1_param_config(struct aw869xx *aw869xx)
{
	aw_dev_info(aw869xx->dev, "%s enter\n", __func__);
...
static void aw869xx_haptic_trig2_param_config(struct aw869xx *aw869xx)
{
	aw_dev_info(aw869xx->dev, "%s enter\n", __func__);
...
static void aw869xx_haptic_trig3_param_config(struct aw869xx *aw869xx)
{
	aw_dev_info(aw869xx->dev, "%s enter\n", __func__);
...
static int aw869xx_haptic_ram_vbat_comp(struct aw869xx *aw869xx, bool flag)
{
	...
	if (flag) {
		if (aw869xx->ram_vbat_compensate ==
			...
			if (aw869xx->vbat > AW869XX_VBAT_REFER) {
				aw_dev_dbg(aw869xx->dev, "%s: aw869xx->vbat(%dmV) > VBAT_REFER(%dmv) not need to vbat compensate!\n",
					   __func__, aw869xx->vbat, AW869XX_VBAT_REFER);
				return 0;
			}
			temp_gain =
...
static int aw869xx_haptic_get_lra_resistance(struct aw869xx *aw869xx)
{
	...
	aw869xx_i2c_read(aw869xx, AW869XX_REG_SYSCTRL7, &reg_val);
	aw_dev_info(aw869xx->dev, "%s: d2s_gain=%d\n",
...
static void aw869xx_haptic_misc_para_init(struct aw869xx *aw869xx)
{
	...
	/* brk_bst_md */
	if (!aw869xx->info.brk_bst_md)
		aw_dev_err(aw869xx->dev, "%s aw869xx->info.brk_bst_md = 0!\n",
			   __func__);
	aw869xx_i2c_write_bits(aw869xx, AW869XX_REG_CONTCFG1,
			       AW869XX_BIT_CONTCFG1_BRK_BST_MD_MASK,
			       aw869xx->info.brk_bst_md << 1);

	/* d2s_gain */
	if (!aw869xx->info.d2s_gain)
		aw_dev_err(aw869xx->dev, "%s aw869xx->info.d2s_gain = 0!\n",
			   __func__);
	aw869xx_i2c_write_bits(aw869xx, AW869XX_REG_SYSCTRL7,
			       AW869XX_BIT_SYSCTRL7_D2S_GAIN_MASK,
			       aw869xx->info.d2s_gain);
	...
	aw869xx_i2c_write_bits(aw869xx, AW869XX_REG_CONTCFG5,
			       ...
			       aw869xx->info.cont_bst_brk_gain << 4);
...
static int aw869xx_haptic_rtp_play(struct aw869xx *aw869xx)
{
...
static int aw869xx_osc_trim_calculation(struct aw869xx *aw869xx,
					...
					unsigned long int real_time)
{
	...
	if (theory_time == real_time) {
		aw_dev_info(aw869xx->dev, "%s theory_time == real_time: %ld, no need to calibrate!\n",
			    __func__, real_time);
		return 0;
	...
	aw_dev_info(aw869xx->dev,
		    ...
		    __func__, real_time, theory_time);
	aw_dev_info(aw869xx->dev, "%s real_code: %02X, trim_lra: 0x%02X\n",
...
static int aw869xx_rtp_trim_lra_calibration(struct aw869xx *aw869xx)
{
	...
	lra_trim_code = aw869xx_osc_trim_calculation(aw869xx, theory_time,
						     aw869xx->microsecond);
	if (lra_trim_code >= 0) {
...
static int aw869xx_rtp_osc_calibration(struct aw869xx *aw869xx)
{
	...
	aw_dev_info(aw869xx->dev, "%s: rtp file:[%s] size = %dbytes\n",
		    __func__, aw869xx_rtp_name[0], aw869xx_rtp->len);
	...
	while (1) {
		if (!aw869xx_haptic_rtp_get_fifo_afs(aw869xx)) {
#ifdef AW_ENABLE_RTP_PRINT_LOG
			aw_dev_info(aw869xx->dev, "%s not almost_full, aw869xx->rtp_cnt=%d\n",
			...
			if (aw869xx->rtp_cnt != aw869xx_rtp->len) {
				if (aw869xx->timeval_flags == 1) {
					...
					aw869xx->start=ktime_to_us(ktime_get());
		...
		if (osc_int_state & AW869XX_BIT_SYSST2_FF_EMPTY) {
			...
			aw869xx->end=ktime_to_us(ktime_get());
			pr_info
		...
		//do_gettimeofday(&aw869xx->end);
		aw869xx->end=ktime_to_us(ktime_get());
		aw869xx->microsecond = aw869xx->end - aw869xx->start;
		...
		if (aw869xx->microsecond > OSC_CALI_MAX_LENGTH) {
			aw_dev_info(aw869xx->dev, "%s osc trim time out! aw869xx->rtp_cnt %d osc_int_state %02x\n",
...
static void aw869xx_rtp_work_routine(struct work_struct *work)
{
	...
	aw869xx_haptic_ram_vbat_comp(aw869xx, false);
	/* boost voltage */
	if (aw869xx->info.bst_vol_rtp <= AW869XX_MAX_BST_VOL)
		aw869xx_haptic_set_bst_vol(aw869xx, aw869xx->info.bst_vol_rtp);
	else
	...
	aw869xx_rtp->len = rtp_file->size;
	aw_dev_info(aw869xx->dev, "%s: rtp file:[%s] size = %dbytes\n",
		    __func__, aw869xx_rtp_name[aw869xx->rtp_file_num],
		    aw869xx_rtp->len);
	memcpy(aw869xx_rtp->data, rtp_file->data, rtp_file->size);
	mutex_unlock(&aw869xx->rtp_lock);
	release_firmware(rtp_file);
	...
	if (rtp_work_flag) {
		aw869xx_haptic_rtp_play(aw869xx);
	} else {
		/* enter standby mode */
		aw869xx_haptic_stop(aw869xx);
 ...
 *****************************************************/
static int
aw869xx_haptic_audio_ctr_list_insert(struct haptic_audio *haptic_audio,
...
static int
aw869xx_haptic_audio_ctr_list_clear(struct haptic_audio *haptic_audio)
{
...
static int aw869xx_haptic_audio_init(struct aw869xx *aw869xx)
{

	aw_dev_dbg(aw869xx->dev, "%s enter!\n", __func__);
...
static int aw869xx_haptic_audio_off(struct aw869xx *aw869xx)
{
	aw_dev_dbg(aw869xx->dev, "%s enter\n", __func__);

	mutex_lock(&aw869xx->lock);
	aw869xx_haptic_set_gain(aw869xx, 0x80);
	aw869xx_haptic_stop(aw869xx);
...
static int aw869xx_haptic_start(struct aw869xx *aw869xx)
{
	aw_dev_dbg(aw869xx->dev, "%s enter\n", __func__);
...
static void aw869xx_clean_status(struct aw869xx *aw869xx)
{
	aw869xx->audio_ready = false;
	aw869xx->haptic_ready = false;
	aw869xx->rtp_routine_on = 0;
	aw_dev_info(aw869xx->dev, "%s enter\n", __func__);
}

static int aw869xx_haptic_juge_RTP_is_going_on(struct aw869xx *aw869xx)
{
	unsigned char glb_state = 0;
	unsigned char rtp_state = 0;

	aw869xx_i2c_read(aw869xx, AW869XX_REG_GLBRD5, &glb_state);
	if (aw869xx->rtp_routine_on
	    || (glb_state == AW869XX_BIT_GLBRD5_STATE_RTP_GO)) {
		rtp_state = 1;	/*is going on */
		aw_dev_info(aw869xx->dev, "%s: rtp_routine_on\n", __func__);
	}
	return rtp_state;
}

static enum hrtimer_restart aw869xx_haptic_audio_timer_func(struct hrtimer
							    *timer)
{
	struct aw869xx *aw869xx =
	    container_of(timer, struct aw869xx, haptic_audio.timer);
	...
	hrtimer_start(&aw869xx->haptic_audio.timer,
		      ktime_set(aw869xx->haptic_audio.timer_val / 1000000,
				(aw869xx->haptic_audio.timer_val % 1000000) *
...
{
	struct aw869xx *aw869xx =
	    container_of(work, struct aw869xx, haptic_audio.work);
	...
	aw_dev_dbg(aw869xx->dev, "%s enter\n", __func__);
	haptic_audio = &(aw869xx->haptic_audio);
	...
	if (ctr_list_flag == 0)
		aw_dev_info(aw869xx->dev, "%s: ctr list empty\n", __func__);
	if (ctr_list_flag == 1) {
		...
		if (ctr_list_diff_cnt > 2) {
			list_for_each_entry_safe_reverse(p_ctr, p_ctr_bak,
							 &(haptic_audio->
							   ctr_list), list) {
				if ((p_ctr->play == 0)
				    && (AW869XX_HAPTIC_CMD_ENABLE ==
					(AW869XX_HAPTIC_CMD_HAPTIC & p_ctr->
					 cmd))) {
					list_del(&p_ctr->list);
					kfree(p_ctr);
					ctr_list_del_cnt++;
	...
	list_for_each_entry_safe_reverse(p_ctr, p_ctr_bak,
					 &(haptic_audio->ctr_list), list) {
		aw869xx->haptic_audio.ctr.cnt = p_ctr->cnt;
		aw869xx->haptic_audio.ctr.cmd = p_ctr->cmd;
		aw869xx->haptic_audio.ctr.play = p_ctr->play;
		aw869xx->haptic_audio.ctr.wavseq = p_ctr->wavseq;
		aw869xx->haptic_audio.ctr.loop = p_ctr->loop;
		aw869xx->haptic_audio.ctr.gain = p_ctr->gain;
		list_del(&p_ctr->list);
		kfree(p_ctr);
		break;
	}
	...
	/* rtp mode jump */
	rtp_is_going_on = aw869xx_haptic_juge_RTP_is_going_on(aw869xx);
	if (rtp_is_going_on) {
	...
	if (aw869xx->haptic_audio.ctr.cmd == AW869XX_HAPTIC_CMD_ENABLE) {
		if (aw869xx->haptic_audio.ctr.play ==
		    AW869XX_HAPTIC_PLAY_ENABLE) {
			aw_dev_info(aw869xx->dev,
				    "%s: haptic_audio_play_start\n", __func__);
			mutex_lock(&aw869xx->lock);
			aw869xx_haptic_stop(aw869xx);
			aw869xx_haptic_play_mode(aw869xx,
						 AW869XX_HAPTIC_RAM_MODE);

			aw869xx_haptic_set_wav_seq(aw869xx, 0x00,
						   aw869xx->haptic_audio.ctr.
						   wavseq);
			aw869xx_haptic_set_wav_seq(aw869xx, 0x01, 0x00);
			...
			aw869xx_haptic_set_gain(aw869xx,
						aw869xx->haptic_audio.ctr.gain);

			aw869xx_haptic_start(aw869xx);
			mutex_unlock(&aw869xx->lock);
		} else if (AW869XX_HAPTIC_PLAY_STOP ==
			   aw869xx->haptic_audio.ctr.play) {
			mutex_lock(&aw869xx->lock);
			aw869xx_haptic_stop(aw869xx);
			mutex_unlock(&aw869xx->lock);
		} else if (AW869XX_HAPTIC_PLAY_GAIN ==
			   aw869xx->haptic_audio.ctr.play) {
			mutex_lock(&aw869xx->lock);
			aw869xx_haptic_set_gain(aw869xx,
						aw869xx->haptic_audio.ctr.gain);
...
static void aw869xx_haptic_auto_break_mode(struct aw869xx *aw869xx, bool flag)
{
	if (flag) {
		aw869xx_i2c_write_bits(aw869xx, AW869XX_REG_PLAYCFG3,
...
static int aw869xx_haptic_ram_get_f0(struct aw869xx *aw869xx)
{
	...
	aw869xx_haptic_auto_break_mode(aw869xx, true);
	...
	if (brk_en_default)
		...
		aw869xx_haptic_auto_break_mode(aw869xx, false);
...
static int aw869xx_haptic_f0_calibration(struct aw869xx *aw869xx)
{
	...
	if (aw869xx_haptic_cont_get_f0(aw869xx)) {
		...
		/* max and min limit */
		f0_limit = aw869xx->f0;
		aw_dev_info(aw869xx->dev, "%s f0_ref = %d, f0_cali_min = %d, f0_cali_max = %d, f0 = %d\n",
...
static int aw869xx_haptic_one_wire_init(struct aw869xx *aw869xx)
{
	aw_dev_info(aw869xx->dev, "%s: enter\n", __func__);

	aw869xx_i2c_write(aw869xx, AW869XX_REG_GLBCFG2,
			  AW869XX_BIT_START_DLY_2P5MS);
	aw869xx_i2c_write_bits(aw869xx,
			       AW869XX_REG_TRGCFG8,
...
static int aw869xx_haptic_init(struct aw869xx *aw869xx)
{
	...
	aw_dev_info(aw869xx->dev, "%s enter\n", __func__);
	/* haptic audio */
	aw869xx->haptic_audio.delay_val = 1;
	aw869xx->haptic_audio.timer_val = 21318;
	INIT_LIST_HEAD(&(aw869xx->haptic_audio.ctr_list));
	hrtimer_init(&aw869xx->haptic_audio.timer, CLOCK_MONOTONIC,
		     HRTIMER_MODE_REL);
	aw869xx->haptic_audio.timer.function = aw869xx_haptic_audio_timer_func;
	INIT_WORK(&aw869xx->haptic_audio.work,
		  aw869xx_haptic_audio_work_routine);
	mutex_init(&aw869xx->haptic_audio.lock);
	INIT_LIST_HEAD(&(aw869xx->haptic_audio.list));
	aw869xx_clean_status(aw869xx);
	/* haptic init */
	mutex_lock(&aw869xx->lock);
	...
	if (aw869xx->info.is_enabled_i2s) {
		...
		aw869xx_haptic_trig2_param_init(aw869xx);
		aw869xx_haptic_trig3_param_init(aw869xx);
		aw869xx_haptic_trig2_param_config(aw869xx);
		aw869xx_haptic_trig3_param_config(aw869xx);
	}
	/* one wire config */
	if (aw869xx->info.is_enabled_one_wire) {
		aw_dev_info(aw869xx->dev, "%s one wire is enabled!\n",
			    __func__);
		aw869xx_haptic_one_wire_init(aw869xx);
	} else {
		aw869xx_haptic_trig1_param_init(aw869xx);
		aw869xx_haptic_trig1_param_config(aw869xx);
	}
	mutex_unlock(&aw869xx->lock);
				    ...
				    struct device_attribute *attr, char *buf)
{
	...
	len += snprintf(buf + len, PAGE_SIZE - len,
			...
			aw869xx->info.bst_vol_ram, aw869xx->info.bst_vol_rtp);
	return len;
}
				     ...
				     const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%d %d", &databuf[0], &databuf[1]) == 2) {
		aw869xx->info.bst_vol_ram = databuf[0];
		aw869xx->info.bst_vol_rtp = databuf[1];
	}
	return count;
}
				 ...
				 size_t count)
{
	...
	if (sscanf(buf, "%x %x", &databuf[0], &databuf[1]) == 2) {
		if (databuf[0] >= AW869XX_SEQUENCER_SIZE ||
		    databuf[1] > aw869xx->ram.ram_num) {
			aw_dev_err(aw869xx->dev,
				   "%s: input value out of range!\n", __func__);
			return count;
		}
				 ...
				 size_t count)
{
	...
	unsigned int rtp_num_max =
	    sizeof(aw869xx_rtp_name) / AW869XX_RTP_NAME_MAX;
	int rc = 0;

	rc = kstrtouint(buf, 0, &val);
	if (rc < 0) {
		aw_dev_err(aw869xx->dev, "%s kstrtouint fail\n", __func__);
		return rc;
	}
	mutex_lock(&aw869xx->lock);
	aw869xx_haptic_stop(aw869xx);
	...
	if ((val > 0) && (val < rtp_num_max)) {
		...
		aw_dev_info(aw869xx->dev, "%s: aw869xx_rtp_name[%d]: %s\n",
			    __func__, val, aw869xx_rtp_name[val]);
		schedule_work(&aw869xx->rtp_work);
	} else {
		aw_dev_err(aw869xx->dev, "%s input number error:%d\n",
			   __func__, val);
	}
	mutex_unlock(&aw869xx->lock);
	return count;
...
static ssize_t aw869xx_ram_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	cdev_t *cdev = dev_get_drvdata(dev);
	struct aw869xx *aw869xx = container_of(cdev, struct aw869xx, vib_dev);
...
static ssize_t aw869xx_ram_store(struct device *dev,
				 struct device_attribute *attr,
				 const char *buf, size_t count)
{
	cdev_t *cdev = dev_get_drvdata(dev);
...
static ssize_t aw869xx_ram_num_show(struct device *dev,
				       struct device_attribute *attr, char *buf)
{
	cdev_t *cdev = dev_get_drvdata(dev);
	struct aw869xx *aw869xx = container_of(cdev, struct aw869xx, vib_dev);
...
//===============================
static ssize_t aw869xx_cali_value_test_show(struct device *dev,
				 struct device_attribute *attr, char *buf)
{
	//cdev_t *cdev = dev_get_drvdata(dev);
	//struct aw869xx *aw869xx = container_of(cdev, struct aw869xx, vib_dev);
...
static ssize_t aw869xx_cali_value_test_store(struct device *dev,
				  struct device_attribute *attr,
				  const char *buf, size_t count)
{
	//cdev_t *cdev = dev_get_drvdata(dev);
					   ...
					   const char *buf, size_t count)
{
	...
	if (rc < 0)
		return rc;
	aw869xx->info.cont_wait_num = val;
	aw869xx_i2c_write(aw869xx, AW869XX_REG_CONTCFG4, val);
					  ...
					  const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%x %x", &databuf[0], &databuf[1]) == 2) {
		aw869xx->info.cont_drv1_lvl = databuf[0];
		aw869xx->info.cont_drv2_lvl = databuf[1];
		aw869xx_i2c_write_bits(aw869xx, AW869XX_REG_CONTCFG6,
				       AW869XX_BIT_CONTCFG6_DRV1_LVL_MASK,
				       aw869xx->info.cont_drv1_lvl);
		aw869xx_i2c_write(aw869xx, AW869XX_REG_CONTCFG7,
					   ...
					   const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%x %x", &databuf[0], &databuf[1]) == 2) {
		aw869xx->info.cont_drv1_time = databuf[0];
		aw869xx->info.cont_drv2_time = databuf[1];
		aw869xx_i2c_write(aw869xx, AW869XX_REG_CONTCFG8,
				  aw869xx->info.cont_drv1_time);
		aw869xx_i2c_write(aw869xx, AW869XX_REG_CONTCFG9,
				  aw869xx->info.cont_drv2_time);
					   ...
					   const char *buf, size_t count)
{
	...
	if (rc < 0)
		return rc;
	aw869xx->info.cont_brk_time = val;
	aw869xx_i2c_write(aw869xx, AW869XX_REG_CONTCFG10,
			  aw869xx->info.cont_brk_time);
	return count;
}
				  ...
				  const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%d %d %d %d %d %d %d %d %d", &databuf[0], &databuf[1],
		...
		if (databuf[0] < 1 || databuf[0] > 3) {
			aw_dev_info(aw869xx->dev, "%s: input trig_num out of range!\n",
				    __func__);
			return count;
		}
		if (databuf[0] == 1 && aw869xx->info.is_enabled_one_wire) {
			aw_dev_info(aw869xx->dev, "%s: trig1 pin used for one wire!\n",
				    __func__);
			return count;
		}
		if ((databuf[0] == 2 || databuf[0] == 3) &&
		     aw869xx->info.is_enabled_i2s) {
			aw_dev_info(aw869xx->dev, "%s: trig2 and trig3 pin used for i2s!\n",
				    __func__);
			return count;
		}
		if (!aw869xx->ram_init) {
		...
		if (databuf[4] > aw869xx->ram.ram_num ||
		    databuf[6] > aw869xx->ram.ram_num) {
			aw_dev_err(aw869xx->dev,
...
static ssize_t aw869xx_haptic_audio_show(struct device *dev,
					struct device_attribute *attr,
					char *buf)
{
	cdev_t *cdev = dev_get_drvdata(dev);
	...
	len += snprintf(buf+len, PAGE_SIZE-len, "%d\n",
			aw869xx->haptic_audio.ctr.cnt);
	return len;
}

static ssize_t aw869xx_haptic_audio_store(struct device *dev,
					  struct device_attribute *attr,
					  const char *buf, size_t count)
{
	cdev_t *cdev = dev_get_drvdata(dev);
	...
	if (sscanf(buf, "%d %d %d %d %d %d", &databuf[0], &databuf[1],
		...
		aw869xx_haptic_audio_ctr_list_insert(&aw869xx->haptic_audio,
						     hap_ctr);

		if (hap_ctr->cmd == 0xff) {
			aw_dev_info(aw869xx->dev, "%s: haptic_audio stop\n",
				    __func__);
			...
			if (hrtimer_active(&aw869xx->haptic_audio.timer)) {
				...
				hrtimer_start(&aw869xx->haptic_audio.timer,
					      ktime_set(aw869xx->haptic_audio.
							delay_val / 1000000,
							(aw869xx->haptic_audio.
							 delay_val % 1000000) *
							1000),
					      HRTIMER_MODE_REL);
			}
...
static ssize_t aw869xx_haptic_audio_time_show(struct device *dev,
					      struct device_attribute *attr,
					      char *buf)
{
	cdev_t *cdev = dev_get_drvdata(dev);
...
static ssize_t aw869xx_haptic_audio_time_store(struct device *dev,
					       struct device_attribute *attr,
					       const char *buf, size_t count)
{
	cdev_t *cdev = dev_get_drvdata(dev);
...
static ssize_t aw869xx_gun_type_show(struct device *dev,
				    struct device_attribute *attr,
				    char *buf)
{
	cdev_t *cdev = dev_get_drvdata(dev);
...
static ssize_t aw869xx_gun_type_store(struct device *dev,
				     struct device_attribute *attr,
				     const char *buf, size_t count)
{
	cdev_t *cdev = dev_get_drvdata(dev);
	...
	aw_dev_dbg(aw869xx->dev, "%s: value=%d\n", __func__, val);

	mutex_lock(&aw869xx->lock);
	aw869xx->gun_type = val;
	mutex_unlock(&aw869xx->lock);
	return count;
}

static ssize_t aw869xx_bullet_nr_show(struct device *dev,
				      struct device_attribute *attr,
				      char *buf)
{
	cdev_t *cdev = dev_get_drvdata(dev);
...
static ssize_t aw869xx_bullet_nr_store(struct device *dev,
				       struct device_attribute *attr,
				       const char *buf, size_t count)
{
	cdev_t *cdev = dev_get_drvdata(dev);
...
static DEVICE_ATTR(ram_num, S_IWUSR | S_IRUGO, aw869xx_ram_num_show,
		   NULL);
...
static DEVICE_ATTR(cali, S_IWUSR | S_IRUGO, aw869xx_cali_show,
		   aw869xx_cali_store);
static DEVICE_ATTR(cali_value_test, S_IWUSR | S_IRUGO, aw869xx_cali_value_test_show,
		   aw869xx_cali_value_test_store);		   	   
...
static DEVICE_ATTR(haptic_audio, S_IWUSR | S_IRUGO, aw869xx_haptic_audio_show,
		   aw869xx_haptic_audio_store);
static DEVICE_ATTR(haptic_audio_time, S_IWUSR | S_IRUGO,
		   aw869xx_haptic_audio_time_show,
		   aw869xx_haptic_audio_time_store);
static DEVICE_ATTR(gun_type, S_IWUSR | S_IRUGO, aw869xx_gun_type_show,
		   aw869xx_gun_type_store);
static DEVICE_ATTR(bullet_nr, S_IWUSR | S_IRUGO, aw869xx_bullet_nr_show,
		   aw869xx_bullet_nr_store);
...
static void aw869xx_vibrator_work_routine(struct work_struct *work)
{
	...
	aw869xx_haptic_stop(aw869xx);
	if (aw869xx->state) {
		if (aw869xx->activate_mode ==
		    AW869XX_HAPTIC_ACTIVATE_RAM_MODE) {
			aw869xx_haptic_ram_vbat_comp(aw869xx, false);
			aw869xx_haptic_play_mode(aw869xx,
						 AW869XX_HAPTIC_RAM_MODE);
			/* boost voltage */
			if (aw869xx->info.bst_vol_ram <= AW869XX_MAX_BST_VOL)
				aw869xx_haptic_set_bst_vol(aw869xx,
							   aw869xx->
							   info.bst_vol_ram);
			else
				aw869xx_haptic_set_bst_vol(aw869xx,
			...
			aw869xx_haptic_play_go(aw869xx);
			   ...
			   AW869XX_HAPTIC_ACTIVATE_RAM_LOOP_MODE) {
			    ...
			    struct device_node *np)
{
	...
	if (aw869xx->reset_gpio < 0) {
		...
		dev_info(dev, "%s: reset gpio provide ok %d\n", __func__,
			 aw869xx->reset_gpio);
	}
	...
	if (aw869xx->irq_gpio < 0)
		...
		dev_info(dev, "%s: irq gpio provide ok irq = %d.\n", __func__,
			 aw869xx->irq_gpio);

	val = of_property_read_u32(np, "vib_mode", &aw869xx->info.mode);
	if (val != 0)
		aw_dev_info(aw869xx->dev, "%s vib_mode not found\n", __func__);
	val = of_property_read_u32(np, "vib_brk_bst_md",
				   &aw869xx->info.brk_bst_md);
	if (val != 0)
		aw_dev_info(aw869xx->dev, "%s vib_brk_bst_md not found\n",
			    __func__);
	val = of_property_read_u32(np, "vib_f0_ref", &aw869xx->info.f0_ref);
	if (val != 0)
	...
	if (val != 0)
		aw_dev_info(aw869xx->dev, "%s vib_cont_brk_gain not found\n",
			    __func__);
	val =
	    of_property_read_u32(np, "vib_cont_tset", &aw869xx->info.cont_tset);
	if (val != 0)
	...
	if (val != 0)
		aw_dev_info(aw869xx->dev, "%s vib_cont_bemf_set not found\n",
			    __func__);
	val = of_property_read_u32(np, "vib_d2s_gain", &aw869xx->info.d2s_gain);
	if (val != 0)
	...
	aw_dev_info(aw869xx->dev, "%s aw869xx->info.is_enabled_i2s = %d\n",
		    __func__, aw869xx->info.is_enabled_i2s);
	aw869xx->info.is_enabled_one_wire =
			of_property_read_bool(np, "vib_is_enabled_one_wire");
	aw_dev_info(aw869xx->dev, "%s aw869xx->info.is_enabled_one_wire = %d\n",
		    __func__, aw869xx->info.is_enabled_one_wire);
	...
	aw_dev_info(aw869xx->dev, "%s aw869xx->info.brk_bst_md: %d\n",
		    __func__, aw869xx->info.brk_bst_md);
	aw_dev_info(aw869xx->dev, "%s aw869xx->info.bst_vol_default: %d\n",
		    __func__, aw869xx->info.bst_vol_default);
	aw_dev_info(aw869xx->dev, "%s aw869xx->info.bst_vol_ram: %d\n",
		    __func__, aw869xx->info.bst_vol_ram);
	aw_dev_info(aw869xx->dev, "%s aw869xx->info.bst_vol_rtp: %d\n",
		    __func__, aw869xx->info.bst_vol_rtp);

	return 0;
}

#ifdef AW_ENABLE_PIN_CONTROL
static int select_pin_ctl(struct aw869xx *aw869xx, const char *name)
{
	size_t i;
	int rc;

	for (i = 0; i < ARRAY_SIZE(aw869xx->pinctrl_state); i++) {
		const char *n = pctl_names[i];

		if (!strncmp(n, name, strlen(n))) {
			rc = pinctrl_select_state(aw869xx->aw869xx_pinctrl,
						  aw869xx->pinctrl_state[i]);
			if (rc)
				aw_dev_info(aw869xx->dev,
					    "cannot select '%s'\n", name);
			else
	...
	rc = -EINVAL;
	aw_dev_info(aw869xx->dev, "%s:'%s' not found\n", __func__, name);

 exit:
	return rc;
}

static int aw869xx_set_interrupt(struct aw869xx *aw869xx)
{
	int rc = select_pin_ctl(aw869xx, "aw869xx_interrupt_active");
	return rc;
}
#endif
...
{
#ifdef AW_ENABLE_PIN_CONTROL
	int rc = select_pin_ctl(aw869xx, "aw869xx_reset_high");

	usleep_range(5000, 6000);
	rc = select_pin_ctl(aw869xx, "aw869xx_reset_low");
	usleep_range(5000, 6000);
	rc = select_pin_ctl(aw869xx, "aw869xx_reset_high");
	usleep_range(3000, 4000);
#endif

	aw_dev_info(aw869xx->dev, "%s enter\n", __func__);
	if (!aw869xx->enable_pin_control) {
...
static int aw869xx_read_chipid(struct aw869xx *aw869xx)
{
	...
	while (cnt < AW_READ_CHIPID_RETRIES) {
		...
		case AW86905_CHIPID:
			...
			aw869xx->info.is_enabled_i2s = false;
			aw869xx_haptic_softreset(aw869xx);
...
printk("[haptic] %s enter\n", __func__);
	...
	aw869xx->enable_pin_control = 0;
#ifdef AW_ENABLE_PIN_CONTROL
	aw869xx->aw869xx_pinctrl = devm_pinctrl_get(&i2c->dev);
	if (IS_ERR(aw869xx->aw869xx_pinctrl)) {
		if (PTR_ERR(aw869xx->aw869xx_pinctrl) == -EPROBE_DEFER) {
			aw_dev_info(&i2c->dev, "pinctrl not ready\n");
			rc = -EPROBE_DEFER;
			return rc;
		}
		aw_dev_info(&i2c->dev, "Target does not use pinctrl\n");
		aw869xx->aw869xx_pinctrl = NULL;
		rc = -EINVAL;
		return rc;
	}
	for (i = 0; i < ARRAY_SIZE(aw869xx->pinctrl_state); i++) {
		...
		struct pinctrl_state *state =
		    pinctrl_lookup_state(aw869xx->aw869xx_pinctrl, n);
		if (IS_ERR(state)) {
			aw_dev_info(&i2c->dev, "cannot find '%s'\n", n);
			rc = -EINVAL;
		}
		aw_dev_info(&i2c->dev, "found pin control %s\n", n);
		aw869xx->pinctrl_state[i] = state;
		aw869xx->enable_pin_control = 1;
		aw869xx_set_interrupt(aw869xx);
	}
#endif
	...
	ret =
	aw_componet_codec_ops.aw_snd_soc_register_codec(&i2c->dev,
