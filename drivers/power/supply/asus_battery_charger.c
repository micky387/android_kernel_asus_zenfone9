...
#define OEM_SET_SIDE_ID_CHANGE	0x2121
...
int qti_cell1_voltage =3000; //shawn
...
// porting -	if (g_ASUS_hwID < 0) {
// porting -		CHG_DBG("%s. Incorrect HWID : %d", __func__, g_ASUS_hwID);
// porting -		return -1;
// porting -	}
	req_msg.hdr.owner = PMIC_GLINK_MSG_OWNER_OEM;
	req_msg.hdr.type = MSG_TYPE_REQ_RESP;
	req_msg.hdr.opcode = OEM_SET_HWID_TO_ADSP;
//porting -	req_msg.hwid_val = g_ASUS_hwID;
        ...
        if (len == sizeof(*Update_side_vid_msg)) {
            ...
            if (Update_side_vid_msg->side_vid != pre_side_vid) {
                if (Update_side_vid_msg->side_vid == 0x040B) {
					...
					CHG_DBG("%s OEM_SET_SIDE_ID_CHANGE. FANDG_PDID_detect = false\n", __func__);
					#endif
                }
                if (IS_ENABLED(CONFIG_QTI_PMIC_GLINK_CLIENT_DEBUG) && qti_phy_bat)
                    power_supply_changed(qti_phy_bat);
			...
			pr_err("Incorrect response length %zu for OEM_SET_SIDE_ID_CHANGE\n",
                len);
        }
        break;        
    default:
				...
				size_t len)
{
	...
	case OEM_GET_Cell_Voltage_REQ:
		if (len == sizeof(*cell_voltage_msg)) {
			...
			// pr_err("[BAT][CHG] shawn asus_battery_charger, Vcell1:%dmV, Vcell2:%dmV\n", qti_cell1_voltage,qti_cell2_voltage);
			ack_set = true;
...
{
	if (quickchg_extcon == NULL || g_asuslib_init == 0) {
		CHG_DBG("%s: quickchg_extcon or g_asuslib_init not Ready\n", __func__);
		return;
	}
