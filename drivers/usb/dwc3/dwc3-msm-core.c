...
static void dwc3_ext_event_notify(struct dwc3_msm *mdwc)
{
	...
	dev_info(mdwc->dev, "[USB] %s enter: mdwc->inputs:%lx hs_phy_flags:%x\n", __func__,
...
skip_update:
	...
	dev_info(mdwc->dev, "[USB] %s: max_speed:%d hw_supp_speed:%d override_speed:%d\n", __func__,
		dwc3_msm_get_max_speed(mdwc), mdwc->max_hw_supp_speed,
		mdwc->override_usb_speed);
	...
	dev_info(mdwc->dev, "[USB] %s: speed=%d\n", __func__, dwc3_msm_get_max_speed(mdwc));
...
int dwc3_msm_set_dp_mode(struct device *dev, bool dp_connected, int lanes)
{
	...
	dev_info(mdwc->dev, "[USB] %s %d, lanes=%d\n", __func__, dp_connected, lanes);
#ifdef CONFIG_USB_HUB_USB3803_MODULE
		if(current_path==HUB){
...
static int dwc3_msm_debug_init(struct dwc3_msm *mdwc)
{
	...
	mdwc->dwc_ipc_log_ctxt = ipc_log_context_create(NUM_LOG_PAGES,
					dev_name(mdwc->dev), DWC3_MINIDUMP);
	...
	unsigned long event, void *ptr)
{
	...
	if (udev->parent && !udev->parent->parent &&
		...
		if (event == USB_DEVICE_ADD) {
			...
			dev_info(mdwc->dev, "[USB] %s set core clk rate %ld\n", __func__,
				mdwc->core_clk_rate);
...
static int dwc3_otg_start_host(struct dwc3_msm *mdwc, int on)
{
	...
	if (on) {
		...
		clk_set_rate(mdwc->core_clk, mdwc->core_clk_rate);
		dwc3_msm_set_clk_sel(mdwc);
		dev_info(mdwc->dev, "[USB] %s: max_speed=%d\n", __func__, dwc->maximum_speed);
